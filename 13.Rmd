...
![](./assets/media-13/image1-2.png)

그림 1. Modeling carousel [@gabrielsson2006pharmacokinetic]

잔차는 관측 값에서 예측 값을 뺀 값을 의미하며(그림 2), ε (epsilon)으로 표시한다(ε =
C<sub>observation</sub> – C<sub>prediction</sub>). 잔차가 양수면, 즉 관측값이
예측곡선보다 위에 있으면 과소예측(underestimation) 되었다고 표현하고, 잔차가 음수이면, 즉
관측값이 예측곡선보다 아래에 있으면 과대예측(overestimation) 되었다고 표현한다.

![](./assets/media-13/image5.png)

그림 2. Definition of residual [@gabrielsson2006pharmacokinetic]

잔차는 정규성(normality)과, 등분산성(homoscedasticity) 그리고 독립성(independency)이 있다고
가정을 한다. 정규성 가정에서는 잔차의 분포는 평균이 0이고 분산이 σ<sup>2</sup>인 정규분표를 따른다고
가정한다(ε\~N(0, σ<sup>2</sup>). 이 때문에 잔차분석은, outlier를 찾아내는데 활용될 수
