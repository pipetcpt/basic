...
tibble::tribble(~" ", ~"Numerical", ~"Visual",
                ...
                "파라미터 정확성(accuracy) / 정밀성(precision)",
                "Parameter estimates, Parameter imprecision estimates (by NONMEM), EBE shrinkage, Bootstrap median and confidence interval",\index{shrinkage / 축소}
...
read_csv(txt) %>% 
  ...
  rename(`$\\Delta$Parameters`= 1, `$\\Delta$-2LL` = 2) %>% \index{-2LL}
  kable(escape=FALSE, 
