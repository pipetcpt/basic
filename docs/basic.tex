...
\begin{Shaded}
\begin{Highlighting}[]
\OperatorTok{$}\NormalTok{DATA ..}\OperatorTok{/}\NormalTok{test.csv IGNORE=}\ErrorTok{@}
\end{Highlighting}
\end{Shaded}
...
\protect\hyperlink{cov}{14장} ``NONMEM의 \$COVARIANCE''에서 언급된 NONMEM 아웃풋 파일을 별첨으로 수록하였다. 지면 관계상 작은 글자 크기로 수록되었으나 웹\footnote{\url{http://pipet.or.kr/books/basic}}에서 동일한 내용을 확인할 수 있다.
