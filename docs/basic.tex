...
\else % if luatex or xetex
  ...
  \setmonofont[Scale=MatchLowercase,ScaleAgain=0.8]{Latin Modern Mono}
\fi
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/01-fig-04} \caption{집단의 데이터를 처리하는 세가지 방법 A) 각자에서 얻어진 데이터, B) naive pooled method, C) two-stage method, D) mixed-effects method}\label{fig:three-ways}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/01-fig-05} \caption{측정된 체중의 분포}\label{fig:measured-weight}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/01-fig-06} \caption{혼합효과 모델링의 개념도. NONMEM 매뉴얼(Beal \protect\hyperlink{ref-nonmem}{2018})의 그림에 수식을 추가한 것}\label{fig:nonmem-figure}
\end{figure}
...
\subsection{\texorpdfstring{\$DATA 레코드 작성\index{\$DATA}}{\$DATA 레코드 작성}}\label{data-uxb808uxcf54uxb4dc-uxc791uxc131}}
...
\subsection{\texorpdfstring{\$INPUT 레코드 작성\index{\$INPUT}}{\$INPUT 레코드 작성}}\label{input-uxb808uxcf54uxb4dc-uxc791uxc131}}
...
\begin{example}
\protect\hypertarget{exm:input}{}{\label{exm:input} }\$PROB, \$DATA, \$INPUT 제어구문 작성의 예\index{\$DATA}\index{\$INPUT}
\end{example}
...
\section{\texorpdfstring{PREDPP에 쓰는 데이터셋\index{PRED}\index{PREDPP}}{PREDPP에 쓰는 데이터셋}}\label{predppuxc5d0-uxc4f0uxb294-uxb370uxc774uxd130uxc14b}}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/02-fig-02} \caption{Comparison between dataset for PRED and PREDPP}\label{fig:pred-predpp}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/02-fig-03} \caption{Basic example of dataset for NONMEM}\label{fig:NONMEM-dataset}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.5\linewidth]{figures/02-fig-04} \caption{Example of dataset}\label{fig:example-dataset}
\end{figure}
...
\hypertarget{id}{%
\subsection{\texorpdfstring{ID\index{ID}}{ID}}\label{id}}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/02-fig-06} \caption{Example of dataset with noncontinuous ID arrangement (NOT appropriate)}\label{fig:id-dataset}
\end{figure}
...
\hypertarget{mdv}{%
\subsection{\texorpdfstring{MDV\index{MDV}}{MDV}}\label{mdv}}
...
\hypertarget{evid}{%
\subsection{\texorpdfstring{EVID\index{EVID}\index{ID}}{EVID}}\label{evid}}
...
\hypertarget{time}{%
\subsection{\texorpdfstring{TIME\index{TIME}}{TIME}}\label{time}}
...
TIME 외에 DATE라는 날짜를 기입하는 데이터 항목을 추가하여 사용할 수 있다. 이는 데이터셋 작성 시 필수적인 변수는 아니지만, 여러 날에 거친 이벤트 레코드를 기입하기에 유용하다. 데이터 항목 레이블의 변수 이름은 DATE, DAT1, DAT2, DAT3로 지정되어 있으며, 코드 \ref{exm:date-dataset-chunk}과 같이 해당 변수명에 따라 날짜에 대해 특정 기입 형식을 따른다. DATE 변수를 추가하여 데이터셋에 달력 날짜를 기입하였을 경우(구분자로 \texttt{/\textquotesingle{}\ 또는}-' 사용) 제어구문 파일의 \$INPUT에 반드시 \texttt{DATE=DROP}이라는 제어구문을 입력해야 한다. 이로써 숫자가 아닌(non-numeric) 구분자가 포함된 DATE를 Data Preprocessor가 받아들이지 않도록 제거해야 한다, 제거하지 않을 경우 에러가 일어난다.\index{\$INPUT}\index{DAT1}\index{DAT2}\index{DAT3}\index{DATE}\index{DATE=DROP}\index{DROP}\index{TIME}

\index{TIME}

\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/02-fig-07} \caption{Example of TIME in dataset - Allowed(sequential) vs. NOT allowed(non-sequential)}\label{fig:time-dataset}
\end{figure}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{DATE  month day year\textbackslash{}index\{DATE\}}
\NormalTok{DAT1  day month year\textbackslash{}index\{DAT1\}}
\NormalTok{DAT2  year month day\textbackslash{}index\{DAT2\}}
\NormalTok{DAT3  year day month\textbackslash{}index\{DAT3\}}
\end{Highlighting}
\end{Shaded}

\hypertarget{amt}{%
\subsection{\texorpdfstring{AMT\index{AMT}}{AMT}}\label{amt}}
...
\hypertarget{rate}{%
\subsection{\texorpdfstring{RATE\index{RATE}}{RATE}}\label{rate}}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/02-fig-09} \caption{Example of AMT and RATE in dataset - drug 100mg IV infusion over 1 hr vs. 0.5 h}\label{fig:AMT-RATE}
\end{figure}
...
\hypertarget{addl-ii}{%
\subsection{\texorpdfstring{ADDL \& II\index{ADDL}}{ADDL \& II}}\label{addl-ii}}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/02-fig-10} \caption{Example of Dosing record in dataset - using TIME, AMT vs. using TIME, AMT, ADDL, II}\label{fig:dosing-dataset}
\end{figure}

\hypertarget{ss}{%
\subsection{\texorpdfstring{SS\index{SS}}{SS}}\label{ss}}
...
\hypertarget{cmt}{%
\subsection{\texorpdfstring{CMT\index{CMT}}{CMT}}\label{cmt}}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/03-fig-01} \caption{NM-TRAN 제어구문의 구성\index{NM-TRAN 제어구문 / NM-TRAN control stream}\index{NM-TRAN control stream / NM-TRAN 제어구문}\index{NM-TRAN control stream / NM-TRAN 제어구문}\index{NM-TRAN}}\label{fig:nm-tran}
\end{figure}



\hypertarget{predppuxc640-pred}{%
\section{\texorpdfstring{PREDPP와 PRED\index{PRED}\index{PREDPP}}{PREDPP와 PRED}}\label{predppuxc640-pred}}
...
\OperatorTok{$}\NormalTok{INPUT ID TIME AMT RATE DUR DV MDV WT AGE SEX HT RF ALB CLCR\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{INPUT\}\textbackslash{}index\{AMT\}\textbackslash{}index\{ID\}\textbackslash{}index\{MDV\}\textbackslash{}index\{RATE\}\textbackslash{}index\{TIME\}}
\OperatorTok{$}\NormalTok{DATA test.csv IGNORE=}\ErrorTok{@}\NormalTok{\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{DATA\}\textbackslash{}index\{IGNORE\}}
\OperatorTok{$}\NormalTok{SUBR ADVAN1 TRANS2\textbackslash{}index\{ADVAN1\}\textbackslash{}index\{TRANS\}\textbackslash{}index\{TRANS2\}}
\OperatorTok{$}\NormalTok{PK\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{PK\}}
...
\NormalTok{  S1 =}\StringTok{ }\NormalTok{V\textbackslash{}index\{S1\}}
\OperatorTok{$}\NormalTok{ERROR\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{ERROR\}}
\NormalTok{  IPRED =}\StringTok{ }\NormalTok{F\textbackslash{}index\{IPRED\}\textbackslash{}index\{PRED\}}
\NormalTok{  W     =}\StringTok{ }\KeywordTok{SQRT}\NormalTok{(}\KeywordTok{THETA}\NormalTok{(}\DecValTok{3}\NormalTok{)}\OperatorTok{**}\DecValTok{2} \OperatorTok{+}\StringTok{ }\KeywordTok{THETA}\NormalTok{(}\DecValTok{4}\NormalTok{)}\OperatorTok{**}\DecValTok{2} \OperatorTok{*}\StringTok{ }\NormalTok{IPRED}\OperatorTok{**}\DecValTok{2}\NormalTok{)\textbackslash{}index\{IPRED\}\textbackslash{}index\{PRED\}}
\NormalTok{  IRES  =}\StringTok{ }\NormalTok{DV }\OperatorTok{{-}}\StringTok{ }\NormalTok{IPRED\textbackslash{}index\{IPRED\}\textbackslash{}index\{IRES\}\textbackslash{}index\{PRED\}\textbackslash{}index\{RES\}}
\NormalTok{  IWRES =}\StringTok{ }\NormalTok{IRES }\OperatorTok{/}\StringTok{ }\NormalTok{W\textbackslash{}index\{IRES\}\textbackslash{}index\{IWRES\}\textbackslash{}index\{RES\}\textbackslash{}index\{WRES\}}
\NormalTok{  Y     =}\StringTok{ }\NormalTok{IPRED }\OperatorTok{+}\StringTok{ }\NormalTok{W }\OperatorTok{*}\StringTok{ }\KeywordTok{EPS}\NormalTok{(}\DecValTok{1}\NormalTok{)\textbackslash{}index\{IPRED\}\textbackslash{}index\{PRED\}}
\OperatorTok{$}\NormalTok{THETA\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{THETA\}}
...
\NormalTok{  (}\DecValTok{0}\NormalTok{, }\DecValTok{100}\NormalTok{)}
  ...
  \FloatTok{0.5}
\OperatorTok{$}\NormalTok{OMEGA\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{OMEGA\}}
  \FloatTok{0.02} \FloatTok{0.01}
  \FloatTok{0.02}
\OperatorTok{$}\NormalTok{SIGMA\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{SIGMA\}}
  \DecValTok{1}\NormalTok{ FIX}
\OperatorTok{$}\NormalTok{ESTIMATION NOABORT MAXEVAL=}\DecValTok{9999}\NormalTok{ METHOD=}\DecValTok{1}\NormalTok{ INTER PRINT=}\DecValTok{5}\NormalTok{\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\KeywordTok{ESTIMATION}\NormalTok{ (\textbackslash{}}\OperatorTok{$}\NormalTok{EST)\}\textbackslash{}index\{MAXEVAL\}\textbackslash{}index\{NOABORT\}}
\OperatorTok{$}\NormalTok{TABLE ID TIME AMT RATE MDV DV IPRED IRES CWRES \textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{TABLE\}\textbackslash{}index\{AMT\}\textbackslash{}index\{CWRES\}\textbackslash{}index\{ID\}\textbackslash{}index\{IPRED\}\textbackslash{}index\{IRES\}\textbackslash{}index\{MDV\}\textbackslash{}index\{PRED\}\textbackslash{}index\{RATE\}\textbackslash{}index\{RES\}\textbackslash{}index\{TIME\}\textbackslash{}index\{WRES\}}
\NormalTok{       ONEHEADER NOPRINT FILE=sdtab1001\textbackslash{}index\{FILE=\}\textbackslash{}index\{NOPRINT\}\textbackslash{}index\{ONEHEADER\}}
\OperatorTok{$}\NormalTok{TABLE ID TIME AMT DV CL V ETA1 ETA2 \textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{TABLE\}\textbackslash{}index\{AMT\}\textbackslash{}index\{ID\}\textbackslash{}index\{TIME\}}
\NormalTok{       ONEHEADER NOPRINT FILE=patab1001\textbackslash{}index\{FILE=\}\textbackslash{}index\{NOPRINT\}\textbackslash{}index\{ONEHEADER\}}
\OperatorTok{$}\NormalTok{TABLE ID RF SEX \textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{TABLE\}\textbackslash{}index\{ID\}}
\NormalTok{       ONEHEADER NOPRINT FILE=catab1001\textbackslash{}index\{FILE=\}\textbackslash{}index\{NOPRINT\}\textbackslash{}index\{ONEHEADER\}}
\OperatorTok{$}\NormalTok{TABLE ID WT AGE SEX HT ALB CLCR \textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{TABLE\}\textbackslash{}index\{ID\}}
\NormalTok{       ONEHEADER NOPRINT FILE=cotab1001\textbackslash{}index\{FILE=\}\textbackslash{}index\{NOPRINT\}\textbackslash{}index\{ONEHEADER\}}
\end{Highlighting}
\end{Shaded}
...
\subsection{\texorpdfstring{\$PROBLEM, \$DATA, \$INPUT\index{\$DATA}\index{\$INPUT}\index{\$PROBLEM}}{\$PROBLEM, \$DATA, \$INPUT}}\label{problem-data-input}}
...
\begin{Shaded}
\begin{Highlighting}[]
\OperatorTok{$}\NormalTok{DATA test.csv IGNORE =}\StringTok{ }\ErrorTok{@}\NormalTok{\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{DATA\}\textbackslash{}index\{IGNORE\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\OperatorTok{$}\NormalTok{DATA ..}\OperatorTok{/}\NormalTok{test.csv IGNORE =}\StringTok{ }\ErrorTok{@}\NormalTok{\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{DATA\}\textbackslash{}index\{IGNORE\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\OperatorTok{$}\NormalTok{DATA test.csv IGNORE =}\StringTok{ }\CommentTok{\# IGNORE=(SEX.EQ.1)\textbackslash{}index\{\textbackslash{}$DATA\}\textbackslash{}index\{IGNORE\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\OperatorTok{$}\NormalTok{DATA test.csv IGNORE =}\StringTok{ }\CommentTok{\# IGNORE=(WEIGHT.LE.40)\textbackslash{}index\{\textbackslash{}$DATA\}\textbackslash{}index\{IGNORE\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\OperatorTok{$}\NormalTok{DATA test.csv IGNORE =}\StringTok{ }\CommentTok{\# IGNORE=(WEIGHT.LE.40, SEX.EQ.1)\textbackslash{}index\{\textbackslash{}$DATA\}\textbackslash{}index\{IGNORE\}}
\end{Highlighting}
\end{Shaded}
...
\subsection{\texorpdfstring{\$SUBROUTINE, \$MODEL, \$PK, \$DES\index{\$DES}\index{\$MODEL}\index{\$PK}\index{\$SUBROUTINE}}{\$SUBROUTINE, \$MODEL, \$PK, \$DES}}\label{subroutine-model-pk-des}}
...
\subsubsection{\texorpdfstring{특수 ADVAN\index{특수 ADVAN / specific ADVANs}\index{specific ADVANs / 특수 ADVAN}\index{specific ADVANs / 특수 ADVAN}}{특수 ADVAN}}\label{uxd2b9uxc218-advan}}
...
\begin{verbatim}
## Error: <text>:4:63: unexpected input
## 3:   kable(booktabs=TRUE, align = 'l',
## 4:         caption = "특수 ADVAN과 그에 따른 TRANS 서브루틴") %>%\
##                                                                  ^
\end{verbatim}
...
\begin{example}
\protect\hypertarget{exm:two-comp-pk}{}{\label{exm:two-comp-pk} }2구획 경구 약동학 모델의 \$PK 레코드\index{\$PK}
\end{example}

\begin{Shaded}
\begin{Highlighting}[]
\OperatorTok{$}\NormalTok{SUBROUTINE ADVAN4 TRANS4\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{SUBROUTINE\}\textbackslash{}index\{ADVAN4\}\textbackslash{}index\{TRANS\}}
...
\begin{equation}
\frac{Amount\ (mg)}{Vd\ (L)} \neq \ DV\ (μg/L)
\label{eq:scalingparam}\index{scaling parameter / 척도 파라미터}\index{scaling parameter / 척도 파라미터}
\end{equation}

위의 식 @red(eq:scalingparam)에서 좌변에 1000을 곱하면 단위가 DV와 통일된다. \texttt{S2=V2/1000}이며, 이를 \$PK에 코드 \ref{exm:two-comp-pk}처럼 기술한다. 동일한 모델을 분석할 경우, 사용자가 정의한 일반 ADVAN을 사용한 모델보다 특수 ADVAN 모델의 실행시간이 더 짧다.\index{특수 ADVAN / specific ADVANs}\index{scaling parameter / 척도 파라미터}\index{specific ADVANs / 특수 ADVAN}\index{scaling parameter / 척도 파라미터}\index{specific ADVANs / 특수 ADVAN}\index{\$PK}

\begin{verbatim}
## Error: '\i' is an unrecognized escape in character string starting ""Parameters*, Descriptions
## S1, Scale for the first compartment\i"
\end{verbatim}
...
\subsection{\texorpdfstring{\$THETA, \$OMEGA, \$SIGMA\index{\$OMEGA}\index{\$SIGMA}\index{\$THETA}}{\$THETA, \$OMEGA, \$SIGMA}}\label{theta-omega-sigma}}
...
\begin{Shaded}
\begin{Highlighting}[]
\OperatorTok{$}\KeywordTok{THETA}\NormalTok{ (}\DecValTok{0}\NormalTok{, }\DecValTok{100}\NormalTok{) (}\DecValTok{0}\NormalTok{, }\DecValTok{30}\NormalTok{) (}\DecValTok{0}\NormalTok{, }\FloatTok{0.7}\NormalTok{)\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{THETA\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\OperatorTok{$}\NormalTok{OMEGA }\FloatTok{0.16} \FloatTok{0.25} \FloatTok{0.16}\NormalTok{\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{OMEGA\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\OperatorTok{$}\NormalTok{OMEGA }\KeywordTok{BLOCK}\NormalTok{(}\DecValTok{2}\NormalTok{)\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{OMEGA\}\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{OMEGA BLOCK\}}
\FloatTok{0.16}
\FloatTok{0.01} \FloatTok{0.25}
\OperatorTok{$}\NormalTok{OMEGA }\FloatTok{0.16}\NormalTok{\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{OMEGA\}}
\end{Highlighting}
\end{Shaded}
...
\OperatorTok{$}\NormalTok{ERROR \textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{ERROR\}}
\NormalTok{IPRED =}\StringTok{ }\NormalTok{F \textbackslash{}index\{IPRED\}\textbackslash{}index\{PRED\}}
\NormalTok{IRES =}\StringTok{ }\NormalTok{DV}\OperatorTok{{-}}\NormalTok{IPRED \textbackslash{}index\{IPRED\}\textbackslash{}index\{IRES\}\textbackslash{}index\{PRED\}\textbackslash{}index\{RES\}}
\NormalTok{IWRES =}\StringTok{ }\NormalTok{IRES}\OperatorTok{/}\NormalTok{IPRED \textbackslash{}index\{IPRED\}\textbackslash{}index\{IRES\}\textbackslash{}index\{IWRES\}\textbackslash{}index\{PRED\}\textbackslash{}index\{RES\}\textbackslash{}index\{WRES\}}
...
\OperatorTok{$}\NormalTok{ERROR \textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{ERROR\}}
\NormalTok{IPRED =}\StringTok{ }\NormalTok{F \textbackslash{}index\{IPRED\}\textbackslash{}index\{PRED\}}
\NormalTok{W =}\StringTok{ }\KeywordTok{SQRT}\NormalTok{(}\KeywordTok{THETA}\NormalTok{(}\DecValTok{4}\NormalTok{)}\OperatorTok{**}\DecValTok{2} \OperatorTok{+}\StringTok{ }\KeywordTok{THETA}\NormalTok{(}\DecValTok{5}\NormalTok{)}\OperatorTok{**}\DecValTok{2} \OperatorTok{*}\NormalTok{IPRED}\OperatorTok{**}\DecValTok{2}\NormalTok{) \textbackslash{}index\{IPRED\}\textbackslash{}index\{PRED\}}
\NormalTok{IRES =}\StringTok{ }\NormalTok{DV}\OperatorTok{{-}}\NormalTok{IPRED \textbackslash{}index\{IPRED\}\textbackslash{}index\{IRES\}\textbackslash{}index\{PRED\}\textbackslash{}index\{RES\}}
\NormalTok{IWRES =}\StringTok{ }\NormalTok{IRES}\OperatorTok{/}\NormalTok{W \textbackslash{}index\{IRES\}\textbackslash{}index\{IWRES\}\textbackslash{}index\{RES\}\textbackslash{}index\{WRES\}}
\NormalTok{Y =}\StringTok{ }\NormalTok{IPRED }\OperatorTok{+}\StringTok{ }\NormalTok{W }\OperatorTok{*}\StringTok{ }\KeywordTok{EPS}\NormalTok{(}\DecValTok{1}\NormalTok{)\textbackslash{}index\{IPRED\}\textbackslash{}index\{PRED\}}
\NormalTok{...}
\OperatorTok{$}\KeywordTok{THETA}\NormalTok{ (}\DecValTok{0}\NormalTok{, }\DecValTok{100}\NormalTok{) (}\DecValTok{0}\NormalTok{, }\DecValTok{30}\NormalTok{) (}\DecValTok{0}\NormalTok{, }\FloatTok{0.7}\NormalTok{) }\DecValTok{10} \FloatTok{0.2}\NormalTok{\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{THETA\}}
\NormalTok{...}
\OperatorTok{$}\NormalTok{SIGMA }\DecValTok{1}\NormalTok{ FIX\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{SIGMA\}}
\end{Highlighting}
\end{Shaded}
...
\subsection{\texorpdfstring{\$ESTIMATION, \$SIMULATION\index{\$ESTIMATION (\$EST)}\index{\$SIMULATION (\$SIM)}}{\$ESTIMATION, \$SIMULATION}}\label{estimation-simulation}}

\$ESTIMATION은 파라미터 추정을 위한 추정방법을 선택하고 추정결과를 어떻게 출력할 것인지를 명시하는 레코드이다.\index{\$ESTIMATION (\$EST)} 추정방법에는 1차추정법(first order, FO), 1차 조건부 추정법 (first-order conditional estimation, FOCE), 라플라시안 추정법 등을 비롯하여 다양한 방법들이 있다. 추정방법에 대한 이론적인 설명은 \protect\hyperlink{estimation-methods}{9장}에서 자세히 다루기로 하고 여기서는 제어구문에 사용하는 기본적인 코드와 용어에 대해서 설명한다.
...
\begin{Shaded}
\begin{Highlighting}[]
\OperatorTok{$}\KeywordTok{SIMULATION}\NormalTok{ (seed) ONLYSIM NSUB=}\DecValTok{1000}\NormalTok{\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\KeywordTok{SIMULATION}\NormalTok{ (\textbackslash{}}\OperatorTok{$}\NormalTok{SIM)\}\textbackslash{}index\{NSUB\}\textbackslash{}index\{ONLYSIM\}}
\end{Highlighting}
\end{Shaded}
...
\subsection{\texorpdfstring{\$COVARIANCE, \$TABLE\index{\$COVARIANCE (\$COV, \$COVAR)}\index{\$TABLE}}{\$COVARIANCE, \$TABLE}}\label{covariance-table}}
...
\begin{example}
\protect\hypertarget{exm:table-record}{}{\label{exm:table-record} }\$TABLE 레코드의 예\index{\$TABLE}
\end{example}
...
\OperatorTok{$}\NormalTok{TABLE ID TIME AMT DV MDV IPRED CWRES IWRES}
...
\subsection{\texorpdfstring{특수 ADVAN과 일반 ADVAN\index{특수 ADVAN / specific ADVANs}\index{specific ADVANs / 특수 ADVAN}\index{specific ADVANs / 특수 ADVAN}}{특수 ADVAN과 일반 ADVAN}}\label{uxd2b9uxc218-advanuxacfc-uxc77cuxbc18-advan}}
...
\begin{example}
\protect\hypertarget{exm:advan4}{}{\label{exm:advan4} }2구획 경구 약동학 모델의 제어구문의 예. ADVAN4\index{ADVAN4}
\end{example}
...
\OperatorTok{$}\NormalTok{ERROR\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{ERROR\}}
\NormalTok{  IPRED =}\StringTok{ }\NormalTok{F\textbackslash{}index\{IPRED\}\textbackslash{}index\{PRED\}}
\NormalTok{  W     =}\StringTok{ }\KeywordTok{SQRT}\NormalTok{(}\KeywordTok{THETA}\NormalTok{(}\DecValTok{6}\NormalTok{))}\OperatorTok{**}\DecValTok{2} \OperatorTok{+}\StringTok{ }\KeywordTok{THETA}\NormalTok{(}\DecValTok{7}\NormalTok{)}\OperatorTok{**}\DecValTok{2} \OperatorTok{*}\StringTok{ }\NormalTok{IPRED}\OperatorTok{**}\DecValTok{2}\ErrorTok{)}\NormalTok{\textbackslash{}index\{IPRED\}\textbackslash{}index\{PRED\}}
\NormalTok{  IRES  =}\StringTok{ }\NormalTok{DV }\OperatorTok{{-}}\StringTok{ }\NormalTok{IPRED\textbackslash{}index\{IPRED\}\textbackslash{}index\{IRES\}\textbackslash{}index\{PRED\}\textbackslash{}index\{RES\}}
\NormalTok{  IWRES =}\StringTok{ }\NormalTok{IRES }\OperatorTok{/}\StringTok{ }\NormalTok{W\textbackslash{}index\{IRES\}\textbackslash{}index\{IWRES\}\textbackslash{}index\{RES\}\textbackslash{}index\{WRES\}}
\NormalTok{  Y     =}\StringTok{ }\NormalTok{IPRED }\OperatorTok{+}\StringTok{ }\NormalTok{W }\OperatorTok{*}\StringTok{ }\KeywordTok{EPS}\NormalTok{(}\DecValTok{1}\NormalTok{) \textbackslash{}index\{IPRED\}\textbackslash{}index\{PRED\}}
...
\begin{example}
\protect\hypertarget{exm:advan5}{}{\label{exm:advan5} }2구획 경구 약동학 모델의 제어구문의 예. ADVAN5 \index{ADVAN5}
\end{example}

\begin{Shaded}
\begin{Highlighting}[]
\OperatorTok{$}\NormalTok{SUBROUTINE ADVAN5\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{SUBROUTINE\}\textbackslash{}index\{ADVAN5\}}

\OperatorTok{$}\NormalTok{MODEL\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{MODEL\}}
  \KeywordTok{COMP}\NormalTok{(DEPOT, DEFDOSE)\textbackslash{}index\{COMP\}\textbackslash{}index\{DEFDOS\}}
  \KeywordTok{COMP}\NormalTok{(CENT, DEFOBS)\textbackslash{}index\{COMP\}\textbackslash{}index\{DEFOBS\}\textbackslash{}index\{FO\}}
  \KeywordTok{COMP}\NormalTok{(PERI)\textbackslash{}index\{COMP\}}
...
\begin{example}
\protect\hypertarget{exm:advan6}{}{\label{exm:advan6} }2구획 경구 약동학 모델의 제어구문의 예. ADVAN6 \index{ADVAN6}
\end{example}

\begin{Shaded}
\begin{Highlighting}[]
\OperatorTok{$}\NormalTok{SUBROUTINE ADVAN6 TOL=}\DecValTok{4}\NormalTok{\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{SUBROUTINE\}\textbackslash{}index\{ADVAN6\}\textbackslash{}index\{TOL\}}

\OperatorTok{$}\NormalTok{MODEL\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{MODEL\}}
  \KeywordTok{COMP}\NormalTok{(DEPOT, DEFDOSE)\textbackslash{}index\{COMP\}\textbackslash{}index\{DEFDOS\}}
  \KeywordTok{COMP}\NormalTok{(CENTRAL, DEFOBS)\textbackslash{}index\{COMP\}\textbackslash{}index\{DEFOBS\}\textbackslash{}index\{FO\}}
  \KeywordTok{COMP}\NormalTok{(PERIPH)\textbackslash{}index\{COMP\}}
...
\OperatorTok{$}\NormalTok{DES\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{DES\}}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/03-fig-02} \caption{모약물과 대사체의 약동학 동시 분석 모델}\label{fig:model-parent-metabolite}
\end{figure}
...
\begin{Shaded}
\begin{Highlighting}[]
\OperatorTok{$}\NormalTok{PROBLEM Model of Parent and Metabolite Concentrations\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{PROBLEM\}}
\OperatorTok{$}\NormalTok{INPUT ID TIME DV AMT EVID MDV CMT\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{INPUT\}\textbackslash{}index\{AMT\}\textbackslash{}index\{CMT\}\textbackslash{}index\{EVID\}\textbackslash{}index\{ID\}\textbackslash{}index\{MDV\}\textbackslash{}index\{TIME\}}
\OperatorTok{$}\NormalTok{DATA filename\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{DATA\}}
\OperatorTok{$}\NormalTok{SUBROUTINE ADVAN6 TOL=}\DecValTok{4}\NormalTok{\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{SUBROUTINE\}\textbackslash{}index\{ADVAN6\}\textbackslash{}index\{TOL\}}
\OperatorTok{$}\NormalTok{MODEL\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{MODEL\}}
  \KeywordTok{COMP}\NormalTok{ (DEPOT, DEFDOS)\textbackslash{}index\{COMP\}\textbackslash{}index\{DEFDOS\}}
  \KeywordTok{COMP}\NormalTok{ (CENTPRNT, DEFOBS)\textbackslash{}index\{COMP\}\textbackslash{}index\{DEFOBS\}\textbackslash{}index\{FO\}}
  \KeywordTok{COMP}\NormalTok{ (CENTMETB)\textbackslash{}index\{COMP\}}
\OperatorTok{$}\NormalTok{PK\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{PK\}}
...
\OperatorTok{$}\NormalTok{ERROR\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{ERROR\}}
  \KeywordTok{IF}\NormalTok{ (CMT.EQ}\FloatTok{.2}\NormalTok{) TYPE=}\DecValTok{0}\NormalTok{ ;Parent concentration\textbackslash{}index\{CMT\}}
  \KeywordTok{IF}\NormalTok{ (CMT.EQ}\FloatTok{.3}\NormalTok{) TYPE=}\DecValTok{1}\NormalTok{ ;Metabolite concentration\textbackslash{}index\{CMT\}}
...
\subsection{\texorpdfstring{PRED 모델\index{PRED}}{PRED 모델}}\label{pred-uxbaa8uxb378}}
...
\begin{example}
\protect\hypertarget{exm:linear-model}{}{\label{exm:linear-model} }\$PRED를 이용한 간단한 선형 모델\index{\$PRED}\index{PRED}
\end{example}

\begin{Shaded}
\begin{Highlighting}[]
\OperatorTok{$}\NormalTok{PROB QTc, PRED\textbackslash{}index\{PRED\}}
\OperatorTok{$}\NormalTok{INPUT ID=DROP DQTC=DV CMAX\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{INPUT\}\textbackslash{}index\{DROP\}\textbackslash{}index\{ID\}}
\OperatorTok{$}\NormalTok{DATA data.csv IGNORE=C \textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{DATA\}\textbackslash{}index\{IGNORE\}}
\NormalTok{; Since the header row starts with a C,}
\NormalTok{; that row is dropped}
\OperatorTok{$}\NormalTok{PRED\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{PRED\}\textbackslash{}index\{PRED\}}
...
\OperatorTok{$}\NormalTok{EST PRINT=}\DecValTok{5}\NormalTok{ MAX=}\DecValTok{9999}\NormalTok{ SIG=}\DecValTok{3}\NormalTok{\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\KeywordTok{ESTIMATION}\NormalTok{ (\textbackslash{}}\OperatorTok{$}\NormalTok{EST)\}}
\OperatorTok{$}\NormalTok{THETA\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{THETA\}}
...
\begin{figure}
\includegraphics[width=0.5\linewidth]{figures/04-fig-01} \caption{Example of IV 1-compartment model}\label{fig:1comp-ex}
\end{figure}

\begin{figure}
\includegraphics[width=0.5\linewidth]{figures/04-fig-02} \caption{Example of IV 2-compartment model}\label{fig:2comp-ex}
\end{figure}



TRANS는 각 모델에 상응하는 특정 파라미터를 알려주는 서브루틴인데, ADVAN1 TRANS1은 정맥투여 1구획 모델의 구조를 설명하기 위해, 파라미터 K를 사용하겠다는 의미이고, ADVAN1 TRANS2는 같은 모델 구조에 K대신 CL, V의 파라미터를 사용하여 설명하겠다는 의미이다. 1구획 모델(ADVAN 1, 2)에서는 TRANS2를, 2구획 모델(ADVAN 3, 4)에서는 TRANS4를 주로 사용하며, ADVAN 1\textasciitilde4에서 사용하는 TRANS 서브루틴을 표 \ref{tab:required-param-subroutine}에서 확인할 수 있다. 추가로, 각 ADVAN에서 필수적인 파라미터들과 TRANS와의 관계는 NONMEM User Guide - Part V의 Appendix 1, 2에 잘 설명되어 있고, 그중 ADVAN 1\textasciitilde4 까지의 내용을 표 \ref{tab:standard-pk-models}에서 확인할 수 있다. (Beal \protect\hyperlink{ref-nonmem}{2018})\index{ADVAN1}\index{S1}\index{TRANS}\index{TRANS2}

\begin{verbatim}
## Error: <text>:2:68: unexpected input
## 1: tibble::tribble(
## 2:   ~`ADVAN subroutine`, ~`TRANS subroutine`, ~`Required parameters`,\
##                                                                       ^
\end{verbatim}

\begin{verbatim}
## Error: <text>:2:34: unexpected input
## 1: readxl::read_excel("data-raw/fig-tab.xlsx", sheet="47", range="A2:D31") %>%
## 2:   rename(`ADVAN TRANS` = 1) %>%  \
##                                     ^
\end{verbatim}

ADVAN과 TRANS 서브루틴 조합에서 상기에 설명한 필수 파라미터 외에 다양한 부가적인 파라미터를 정의할 수 있는데, 여기에는 흡수지연시간, 생체이용률, 0차흡수의 속도, 기간 등이 있다. 모든 부가 파라미터에는 파라미터 이름 뒤에 숫자가 나오며, 이는 그 파라미터가 적용되는 구획을 표시한다. 해당 내용은 표 \ref{tab:additional-param}에 간략히 설명하였으며 NONMEM User Guide - Part V의 Appendix 1 및 Part VI - Chapter VII에 자세히 설명되어 있다. (Beal \protect\hyperlink{ref-nonmem}{2018})\index{TRANS}

\begin{verbatim}
## Error: <text>:6:51: unexpected input
## 5: Dn, Duration parameter for compartment n
## 6: ALAGn, Absorption lag parameter for compartment n"\
##                                                      ^
\end{verbatim}

앞서 언급된 ADVAN \& TRANS 서브루틴 및 필수/부가 파라미터를 실제 제어구문의 예를 들어 간략히 설명하고자 한다. \$SUBROUTINES 문법은 아래 예시와 같이 쓸 수 있는데 이는 정맥 외 투여 1차 흡수, 2구획 모델이며 CL, V2, V3, Q, Ka의 파라미터를 사용하여 설명하겠다는 의미이다. \$PK block에는 \$SUBROUTINES에서 사용하고자 하는 약동학 파라미터들을 정의하는 곳으로 \texttt{CL\ =\ THETA(1)\ *\ EXP(ETA(1))} 와 같은 형식으로 표기할 수도 있고, 아래 예시와 같이 표기할 수도 있다. 아래 예시처럼 표기하게 되면 추후 공변량에 대한 수식을 만들게 될 때, 보다 수월하게 작성할 수 있는데, 공변량은 THETA에 수식을 붙이는 형태이기 때문이다. 이는 추후 \protect\hyperlink{covariate-analysis}{공변량 분석}에서 다뤄질 예정이다.\index{\$PK}\index{\$SUBROUTINE}\index{TRANS}
...
\begin{Shaded}
\begin{Highlighting}[]
\OperatorTok{$}\NormalTok{SUBROUTINES ADVAN4 TRANS4\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{SUBROUTINE\}\textbackslash{}index\{ADVAN4\}\textbackslash{}index\{TRANS\}}
\OperatorTok{$}\NormalTok{PK \textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{PK\}}
\NormalTok{  TVCL =}\StringTok{ }\KeywordTok{THETA}\NormalTok{(}\DecValTok{1}\NormalTok{)}
...
\NormalTok{  ALAG1=}\StringTok{  }\KeywordTok{THETA}\NormalTok{(}\DecValTok{6}\NormalTok{) }\OperatorTok{*}\StringTok{ }\KeywordTok{EXP}\NormalTok{(}\KeywordTok{ETA}\NormalTok{(}\DecValTok{6}\NormalTok{))\textbackslash{}index\{ALAG\}}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/04-fig-03} \caption{Structure of 2-compartment extravascular model}\label{fig:2comp-structure}
\end{figure}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{IF}\NormalTok{ (DOSE.EQ}\FloatTok{.50}\NormalTok{) F1 =}\StringTok{ }\DecValTok{1}\NormalTok{\textbackslash{}index\{F1\}}
\KeywordTok{IF}\NormalTok{ (DOSE.EQ}\FloatTok{.100}\NormalTok{) F1 =}\StringTok{ }\KeywordTok{THETA}\NormalTok{(}\DecValTok{7}\NormalTok{)\textbackslash{}index\{F1\}}
\KeywordTok{IF}\NormalTok{ (DOSE.EQ}\FloatTok{.200}\NormalTok{) F1 =}\StringTok{ }\KeywordTok{THETA}\NormalTok{(}\DecValTok{8}\NormalTok{)\textbackslash{}index\{F1\}}
\end{Highlighting}
\end{Shaded}
...
\centering
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{2cm}>{\raggedright\arraybackslash}p{2.5cm}>{\raggedright\arraybackslash}p{1.5cm}>{\raggedright\arraybackslash}p{7.0cm}}
\toprule
...
## 1: advan <- c("ADVAN5 (General Linear)",\
##                                          ^
\end{verbatim}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/05-fig-01} \caption{일반 선형 ADVAN을 이용한 코드의 구조}\label{fig:advan-code}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/05-fig-02} \caption{일반 선형 ADVAN을 이용한 코드 예시}\label{fig:advan-example}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/05-fig-03} \caption{일반 비선형 ADVAN을 이용한 코드의 구조}\label{fig:nonlin-code}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/05-fig-04} \caption{일반 비선형 ADVAN을 이용한 코드 예시}\label{fig:nonlin-advan-code-example}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/05-fig-05} \caption{\$DES 구문 적용을 위한 코드 예시\index{\$DES}}\label{fig:des-example}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/05-fig-06} \caption{2구획 경구모델}\label{fig:2comp-oral}
\end{figure}
...
\OperatorTok{$}\NormalTok{MODEL\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{MODEL\}}
  ...
  \KeywordTok{COMP}\NormalTok{(CENTRAL, DEFOBS)\textbackslash{}index\{COMP\}\textbackslash{}index\{DEFOBS\}\textbackslash{}index\{FO\}}
  \KeywordTok{COMP}\NormalTok{(PERIPH)\textbackslash{}index\{COMP\}}
\OperatorTok{$}\NormalTok{PK ; Define basic PK relationships\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{PK\}}
...
\NormalTok{  K23=}\StringTok{ }\NormalTok{Q}\OperatorTok{/}\NormalTok{V2}
\NormalTok{  K32=}\StringTok{ }\NormalTok{Q}\OperatorTok{/}\NormalTok{V3}
\OperatorTok{$}\NormalTok{DES ; Define differential equations\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{DES\}}
...
\section{\texorpdfstring{ADVAN9 - General Nonlinear Model with Equilibrium Compartments\index{ADVAN9}}{ADVAN9 - General Nonlinear Model with Equilibrium Compartments}}\label{advan9---general-nonlinear-model-with-equilibrium-compartments}}
...
\section{\texorpdfstring{ADVAN10 - One Compartment Model with Michaelis-Menten Elimination\index{ADVAN1}\index{ADVAN10}}{ADVAN10 - One Compartment Model with Michaelis-Menten Elimination}}\label{advan10---one-compartment-model-with-michaelis-menten-elimination}}
...
\section{\texorpdfstring{ADVAN11과 12 - Three Compartment Linear Model (IV and First Order Absorption) \index{ADVAN1}\index{ADVAN11}}{ADVAN11과 12 - Three Compartment Linear Model (IV and First Order Absorption) }}\label{advan11uxacfc-12---three-compartment-linear-model-iv-and-first-order-absorption}}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/06-fig-01} \caption{ADVAN11(위)과 ADVAN12(아래)의 모델 구조와 micro constants의 이름들}\label{fig:advan1112}
\end{figure}

\hypertarget{advan13---general-nonlinear-model-using-lsoda}{%
\section{\texorpdfstring{ADVAN13 - General Nonlinear Model using LSODA\index{ADVAN1}\index{ADVAN13}}{ADVAN13 - General Nonlinear Model using LSODA}}\label{advan13---general-nonlinear-model-using-lsoda}}
...
\hypertarget{pred-advanuxc744-uxc4f0uxc9c0-uxc54auxb294-uxcf54uxb529-pred}{%
\chapter{\texorpdfstring{\$PRED: ADVAN을 쓰지 않는 코딩 \{\#PRED\}\index{\$PRED}\index{PRED}}{\$PRED: ADVAN을 쓰지 않는 코딩 \{\#PRED\}}}\label{pred-advanuxc744-uxc4f0uxc9c0-uxc54auxb294-uxcf54uxb529-pred}}

임동석

\hypertarget{preduxc640-predpp-libraryuxc0acuxc6a9uxd560-uxb54cuxc758-uxcc28uxc774}{%
\section{\texorpdfstring{\$PRED와 PREDPP library사용할 때의 차이\index{\$PRED}\index{PRED}\index{PREDPP}}{\$PRED와 PREDPP library사용할 때의 차이}}\label{preduxc640-predpp-libraryuxc0acuxc6a9uxd560-uxb54cuxc758-uxcc28uxc774}}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/07-fig-01} \caption{1구획 모델의 ADVAN2, \$PRED 제어구문 파일 비교\index{\$PRED}\index{ADVAN2}\index{PRED}}\label{fig:first-order-pred}
\end{figure}



\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/07-fig-02} \caption{ADVAN과 \$PRED의 데이터셋 차이\index{\$PRED}\index{PRED}}\label{fig:pred-dataset}
\end{figure}
...
\section{\texorpdfstring{\$PRED를 써야 하는 경우\index{\$PRED}\index{PRED}}{\$PRED를 써야 하는 경우}}\label{preduxb97c-uxc368uxc57c-uxd558uxb294-uxacbduxc6b0}}
...
\OperatorTok{$}\NormalTok{DATA ..}\OperatorTok{/}\NormalTok{sigmoidal\_effect.csv IGNORE=}\ErrorTok{@}\NormalTok{\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{DATA\}\textbackslash{}index\{IGNORE\}}
\OperatorTok{$}\NormalTok{INPUT ID CP RESP=DV\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{INPUT\}\textbackslash{}index\{ID\}\textbackslash{}index\{RES\}}
\OperatorTok{$}\NormalTok{PRED\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{PRED\}\textbackslash{}index\{PRED\}}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/08-fig-01} \caption{Newton-Raphson 방법을 이용한 함수의 최소값 추정 알고리즘의 개념도}\label{fig:newton-minimization}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/08-fig-02} \caption{복잡한 함수에서 Newton-Raphson 방법의 한계}\label{fig:limitation-newton-rhapson}
\end{figure}
...
\begin{verbatim}
## Error: <text>:2:77: unexpected input
## 1: #knitr::include_graphics("assets/media-08/08-fig-table-1.png")
## 2: readxl::read_excel("data-raw/fig-tab.xlsx", sheet="71", range="B2:F19") %>% \
##                                                                                ^
\end{verbatim}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/08-fig-03} \caption{탐색적 데이터 분석을 통한 IE 값 결정}\label{fig:eda-ie}
\end{figure}
...
\hypertarget{noabort}{%
\subsection{\texorpdfstring{NOABORT\index{NOABORT}}{NOABORT}}\label{noabort}}
...
\hypertarget{maxeval}{%
\subsection{\texorpdfstring{MAXEVAL\index{MAXEVAL}}{MAXEVAL}}\label{maxeval}}
...
\begin{verbatim}
## Error: <text>:2:39: unexpected input
## 1: library(tibble)
## 2: fofoce <- tribble(~`비교`, ~FO, ~FOCE,\
##                                          ^
\end{verbatim}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/09-fig-01} \caption{NONMEM 추정 도식화}\label{fig:obj}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/09-fig-02} \caption{ordinary least square}\label{fig:OLS}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/09-fig-03} \caption{동전 앞면이 7회 나올 확률의 가능도}\label{fig:coin}
\end{figure}
...
\begin{verbatim}
## Error: <text>:2:74: unexpected input
## 1: read_csv('data-raw/media-10-tab-1.csv', col_types="cccccc") %>% 
## 2:   kable(booktabs=TRUE, caption = "투여경로에 따라 CMT가 기록된 데이터셋")\
##                                                                             ^
\end{verbatim}
...
다음으로, \$TABLE을 통해 생성된 표 파일들을 이용하여 진단 플롯을 그리는 방법을 알아보도록 하자. \$TABLE은 코드 \ref{exm:table-example}와 같이 많이 쓰는데, 이를 통해 생성되는 표 파일들과 R의 xpose4 package를 이용하여 진단 플롯을 그릴 수 있다. (Hooker et al. \protect\hyperlink{ref-R-xpose4}{2020})\index{\$TABLE}
...
\begin{Shaded}
\begin{Highlighting}[]
\OperatorTok{$}\NormalTok{TABLE ID TIME AMT DV MDV DV IPRED CWRES IWRES ONEHEADER NOPRINT FILE=sdtab3\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{TABLE\}\textbackslash{}index\{AMT\}\textbackslash{}index\{CWRES\}\textbackslash{}index\{FILE=\}\textbackslash{}index\{ID\}\textbackslash{}index\{IPRED\}\textbackslash{}index\{IWRES\}\textbackslash{}index\{MDV\}\textbackslash{}index\{NOPRINT\}\textbackslash{}index\{ONEHEADER\}\textbackslash{}index\{PRED\}\textbackslash{}index\{RES\}\textbackslash{}index\{TIME\}\textbackslash{}index\{WRES\}}
\OperatorTok{$}\NormalTok{TABLE ID }\KeywordTok{ETA}\NormalTok{(}\DecValTok{1}\NormalTok{) }\KeywordTok{ETA}\NormalTok{(}\DecValTok{2}\NormalTok{)                        ONEHEADER NOPRINT FILE=patab3\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{TABLE\}\textbackslash{}index\{FILE=\}\textbackslash{}index\{ID\}\textbackslash{}index\{NOPRINT\}\textbackslash{}index\{ONEHEADER\}}
\OperatorTok{$}\NormalTok{TABLE ID SEX                                  ONEHEADER NOPRINT FILE=catab3\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{TABLE\}\textbackslash{}index\{FILE=\}\textbackslash{}index\{ID\}\textbackslash{}index\{NOPRINT\}\textbackslash{}index\{ONEHEADER\}}
\OperatorTok{$}\NormalTok{TABLE ID AGE WT HT                            ONEHEADER NOPRINT FILE=cotab3\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{TABLE\}\textbackslash{}index\{FILE=\}\textbackslash{}index\{ID\}\textbackslash{}index\{NOPRINT\}\textbackslash{}index\{ONEHEADER\}}
\end{Highlighting}
\end{Shaded}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./assets/media-10/image11} \caption{Basic goodness-of-fit Plots}\label{fig:basic-gof}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./assets/media-10/image12} \caption{Individual Plots}\label{fig:indi-plots}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/11-fig-01} \caption{공변량 분석의 workflow}\label{fig:workflow-covariate}
\end{figure}
...
이 단계의 핵심적 절차는 각 항목이 어떠한 척도로 조사되었는가를 확인하고, 해당 척도에 적합한 방법으로 기술 통계 분석을 실시하는 것이다. 즉, 비척도로 조사된 항목은 평균, 표준편차, 분위 수 등의 값을 산출하여야 하고, 서열척도나 명목척도로 조사된 항목은 각 수준(level) 별로 빈도를 파악한다. 등간척도의 경우에는 상황에 따라 적절한 기술통계 방법을 선택한다. 실제 모델링 수행 시에는 완성된 기반 모델 (base model)의 제어구문 맨 뒤에 \$TABLE로서 잠재적 공변량의 상세 사항을 출력한 후, R package인 xpose4 (ver. 4.5.X 기준) (Hooker et al. \protect\hyperlink{ref-R-xpose4}{2020})를 사용하면, 6: Covariate model \textgreater{} 2. Numerically summarize the covariates 기능을 이용하여 이 과정을 간단히 수행할 수 있다. (구체적인 방법은 xpose4 매뉴얼\footnote{\url{https://cran.r-project.org/web/packages/xpose4/xpose4.pdf}} 참조) 이러한 분석의 결과로는 다음과 같은 표를 얻을 수 있다.\index{\$TABLE}

\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/11-fig-02} \caption{항목 평가에 필요한 잠재적 공변량 요약표}\label{fig:covariate-summary}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/11-fig-03} \caption{독립 변수의 변화에 따른 토리여과율 예측값과 실제값의 차이: 연령의 예시}\label{fig:gfr}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/11-fig-04} \caption{부적절한 데이터 분포}\label{fig:improper-distribution}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/11-fig-05} \caption{공변량 간의 상관 관계 탐색을 위한 scatterplot matrix}\label{fig:cov-scatter}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/11-fig-06} \caption{시각적 스크리닝을 위한 plot 예시}\label{fig:visual-screening}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/11-fig-07} \caption{비선형적 상관관계를 가질 가능성이 있는 경우 (Owen \protect\hyperlink{ref-kelly}{2014})}\label{fig:nonlinear-rel}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/11-fig-08} \caption{GAM에서 1개의 step을 실행한 결과}\label{fig:gam-step}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/11-fig-09} \caption{GAM 실행 summary}\label{fig:gam-summary}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/11-fig-10} \caption{임의효과로 남겨져 있는 설명 불가능한 개인차}\label{fig:unnamed-chunk-1}
\end{figure}
...
\subsection{\texorpdfstring{공변량 평가: 후진제거(backward-elimination)\index{후진제거 / backward elimination}\index{backward elimination / 후진제거}\index{backward elimination / 후진제거}}{공변량 평가: 후진제거(backward-elimination)}}\label{uxacf5uxbcc0uxb7c9-uxd3c9uxac00-uxd6c4uxc9c4uxc81cuxac70backward-elimination}}
...
\hypertarget{part-uxbaa8uxb378-uxc9c4uxb2e8-uxbc0f-uxc57duxb3d9uxd559-uxc57duxb825uxd559-uxc5f0uxacc4-uxbaa8uxb378}{%
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/12-fig-01} \caption{모델링-시뮬레이션 workflow 및 구성 요소}\label{fig:ms-workflow}
\end{figure}
...
\begin{verbatim}
## Error: <text>:8:141: unexpected input
## 7:                 "파라미터 정확성(accuracy) / 정밀성(precision)",
## 8:                 "Parameter estimates, Parameter imprecision estimates (by NONMEM), EBE shrinkage, Bootstrap median and confidence interval",\
##                                                                                                                                                ^
\end{verbatim}
...
\begin{equation}
AIC = -2LL + 2k \ \ \text{(k = number of parameters)}\index{-2LL}
\label{eq:aic}
\end{equation}
...
\begin{verbatim}
## Error: <text>:8:61: unexpected input
## 7:   mutate_all(as.character) %>% 
## 8:   rename(`$\\Delta$Parameters`= 1, `$\\Delta$-2LL` = 2) %>% \
##                                                                ^
\end{verbatim}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/13-fig-01} \caption{Modeling carousel (Gabrielsson \protect\hyperlink{ref-gabrielsson}{2006})}\label{fig:carousel}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.5\linewidth]{./figures/13-fig-02} \caption{Definition of residual (Gabrielsson \protect\hyperlink{ref-gabrielsson}{2006})}\label{fig:def-residual}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/13-fig-03} \caption{Relationship between standard deviation of residual variability and predicted concentrations}\label{fig:pred-sd}
\end{figure}
...
\OperatorTok{$}\NormalTok{ERROR\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{ERROR\}}
\NormalTok{  IPRED =}\StringTok{ }\NormalTok{F\textbackslash{}index\{IPRED\}\textbackslash{}index\{PRED\}}
\NormalTok{  Y     =}\StringTok{ }\NormalTok{IPRED }\OperatorTok{*}\StringTok{ }\NormalTok{(}\DecValTok{1}\OperatorTok{+}\KeywordTok{EPS}\NormalTok{(}\DecValTok{1}\NormalTok{)) }\OperatorTok{+}\StringTok{ }\KeywordTok{EPS}\NormalTok{(}\DecValTok{2}\NormalTok{)\textbackslash{}index\{IPRED\}\textbackslash{}index\{PRED\}}
\end{Highlighting}
\end{Shaded}
...
\OperatorTok{$}\NormalTok{ERROR\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{ERROR\}}
\NormalTok{  IPRED =}\StringTok{ }\NormalTok{F\textbackslash{}index\{IPRED\}\textbackslash{}index\{PRED\}}
\NormalTok{  W     =}\StringTok{ }\KeywordTok{SQRT}\NormalTok{(}\KeywordTok{THETA}\NormalTok{(}\DecValTok{5}\NormalTok{)}\OperatorTok{**}\DecValTok{2} \OperatorTok{+}\StringTok{ }\KeywordTok{THETA}\NormalTok{(}\DecValTok{6}\NormalTok{) }\OperatorTok{**}\DecValTok{2} \OperatorTok{*}\StringTok{ }\NormalTok{IPRED}\OperatorTok{**}\DecValTok{2}\NormalTok{)\textbackslash{}index\{IPRED\}\textbackslash{}index\{PRED\}}
\NormalTok{  IRES  =}\StringTok{ }\NormalTok{DV }\OperatorTok{{-}}\StringTok{ }\NormalTok{IPRED\textbackslash{}index\{IPRED\}\textbackslash{}index\{IRES\}\textbackslash{}index\{PRED\}\textbackslash{}index\{RES\}}
\NormalTok{  IWRES =}\StringTok{ }\NormalTok{IRES }\OperatorTok{/}\StringTok{ }\NormalTok{W\textbackslash{}index\{IRES\}\textbackslash{}index\{IWRES\}\textbackslash{}index\{RES\}\textbackslash{}index\{WRES\}}
\NormalTok{  Y     =}\StringTok{ }\NormalTok{IPRED }\OperatorTok{+}\StringTok{ }\NormalTok{W }\OperatorTok{*}\StringTok{ }\KeywordTok{EPS}\NormalTok{(}\DecValTok{1}\NormalTok{)\textbackslash{}index\{IPRED\}\textbackslash{}index\{PRED\}}
\end{Highlighting}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/13-fig-04} \caption{Example of Run test (Gabrielsson \protect\hyperlink{ref-gabrielsson}{2006})}\label{fig:run-test}
\end{figure}
...
Xpose4 package를 이용하여 여러 진단 플롯들을 그릴 수 있는데, 그 중 하나인 basic goodness-of-fit plots에서(그림 \ref{fig:basic-gof-plot}) 아래쪽 두 개의 그림이 잔차(residual)에 대한 진단 플롯이다. 좌측은 \textbar IWRES\textbar{} vs Individual predictions (IPRED), 우측은 Conditional weighted residuals (CWRES) vs Time을 나타낸 그림이다. IWRES는 개인 가중잔차(Individual Weighted residuals)이고 \textbar IWRES\textbar 는 개인 가중잔차의 절대값이다. 잔차모델이 적절하다면, CWRES 플롯의 모든 점들이 0을 기준으로 대칭으로 분포하게 되며, CWRES 값들의 흩어진 정도가 시간이나 개인 예측 값에 따라 경향성이 없고, 그 범위가 -3과+3 사이에 분포할 것으로 기대된다(\protect\hyperlink{use-of-xpose4}{10장} 참고).\index{개인 예측 / individual(specific) prediction(s)}\index{individual(specific) prediction(s) / 개인 예측}\index{잔차 / residual error}\index{residual error / 잔차}\index{conditional weighted residuals / 조건부 가중 잔차}\index{CWRES}\index{IPRED}\index{IWRES}\index{PRED}\index{RES}\index{WRES}

\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/13-fig-05} \caption{Basic goodness-of-fit plots}\label{fig:basic-gof-plot}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/13-fig-06} \caption{Schematic illustration of wrong structural models (upper row) and wrong variance models (lower row) (Gabrielsson \protect\hyperlink{ref-gabrielsson}{2006})}\label{fig:wrong-models}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/13-fig-07} \caption{Example of Wrong structure model (lag time) (Gabrielsson \protect\hyperlink{ref-gabrielsson}{2006})}\label{fig:wrong-structure}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/13-fig-08} \caption{Lag time 추가시의 GOF 변화}\label{fig:lag-gof}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/13-fig-09} \caption{Example of Wrong structure model (2-compartment model)}\label{fig:u-curve}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./figures/13-fig-10} \caption{Example of Wrong variance model (error model)}\label{fig:addi-comb-gof}
\end{figure}
...
\toprule
\multicolumn{1}{c}{ } & \multicolumn{3}{c}{Plot of $epsilon_{i}$ versus} \\
\cmidrule(l{3pt}r{3pt}){2-4}
...
\hypertarget{nonmemuxc758-covariance-cov}{%
\chapter{\texorpdfstring{NONMEM의 \$COVARIANCE \{\#cov\}\index{\$COVARIANCE (\$COV, \$COVAR)}}{NONMEM의 \$COVARIANCE \{\#cov\}}}\label{nonmemuxc758-covariance-cov}}
...
\OperatorTok{$}\NormalTok{INPUT ID AMT=DROP TIME DV BWT\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{INPUT\}\textbackslash{}index\{AMT\}\textbackslash{}index\{DROP\}\textbackslash{}index\{ID\}\textbackslash{}index\{TIME\}}
\OperatorTok{$}\NormalTok{DATA ..}\OperatorTok{/}\NormalTok{THEO\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{DATA\}}
\OperatorTok{$}\NormalTok{PRED\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{PRED\}\textbackslash{}index\{PRED\}}
...
\OperatorTok{$}\KeywordTok{THETA}\NormalTok{ (}\DecValTok{0}\NormalTok{, }\DecValTok{2}\NormalTok{) (}\DecValTok{0}\NormalTok{, }\DecValTok{50}\NormalTok{) (}\DecValTok{0}\NormalTok{, }\FloatTok{0.1}\NormalTok{)\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{THETA\}}
\OperatorTok{$}\NormalTok{OMEGA }\KeywordTok{BLOCK}\NormalTok{(}\DecValTok{3}\NormalTok{)\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{OMEGA\}\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{OMEGA BLOCK\}}
  \FloatTok{0.2}
  \FloatTok{0.1}  \FloatTok{0.2}
  \FloatTok{0.1}  \FloatTok{0.1}  \FloatTok{0.2}
\OperatorTok{$}\NormalTok{SIGMA }\FloatTok{0.1}  \FloatTok{0.1}\NormalTok{\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{SIGMA\}}
\OperatorTok{$}\NormalTok{EST MAX=}\DecValTok{9999}\NormalTok{ METHOD=ZERO POSTHOC\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\KeywordTok{ESTIMATION}\NormalTok{ (\textbackslash{}}\OperatorTok{$}\NormalTok{EST)\}\textbackslash{}index\{POSTHOC\}}
\OperatorTok{$}\NormalTok{COV PRINT=ERS\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\KeywordTok{COVARIANCE}\NormalTok{ (\textbackslash{}}\OperatorTok{$}\NormalTok{COV, \textbackslash{}}\OperatorTok{$}\NormalTok{COVAR)\}\textbackslash{}index\{PRINT=E\}}
\OperatorTok{$}\NormalTok{TAB ID TIME IPRE CWRES\textbackslash{}index\{CWRES\}\textbackslash{}index\{ID\}\textbackslash{}index\{RES\}\textbackslash{}index\{TIME\}\textbackslash{}index\{WRES\}}
\NormalTok{     FILE=sdtab NOPRINT ONEHEADER\textbackslash{}index\{FILE=\}\textbackslash{}index\{NOPRINT\}\textbackslash{}index\{ONEHEADER\}}
\OperatorTok{$}\NormalTok{TAB ID }\KeywordTok{ETA}\NormalTok{(}\DecValTok{1}\NormalTok{) }\KeywordTok{ETA}\NormalTok{(}\DecValTok{2}\NormalTok{) }\KeywordTok{ETA}\NormalTok{(}\DecValTok{3}\NormalTok{)\textbackslash{}index\{ID\}}
\NormalTok{     FILE=IndiEta.txt NOAPPEND ONEHEADER FIRSTONLY\textbackslash{}index\{FILE=\}\textbackslash{}index\{FIRSTONLY\}\textbackslash{}index\{NOAPPEND\}\textbackslash{}index\{ONEHEADER\}}
\end{Highlighting}
\end{Shaded}

위의 \$COV STEP에 의해 생성된 부분은 별첨 1과 같으며, 원래 output은 126열까지까지 있으나, 지면의 제약으로 인해 104열이후는 생략하였다.\index{\$COVARIANCE (\$COV, \$COVAR)}

\$COV에 의해 생성되는 첫 번째 부분(section)은 추정치의 표준오차 부분으로 출력의 직전 section인 FINAL PARAMETER ESTIMATE과 배치 모양이 똑같다. 이후의 출력들은 대개 이를 구하기 위해 필요한 중간계산결과들이다.\index{\$COVARIANCE (\$COV, \$COVAR)}
...
\$COV output의 다섯 번째 부분은 상관행렬로부터 구한 고유치(eigenvalue)들을 작은 것부터 순서대로 나열한 것이다. 만약 상관행렬이 positive definite(양정치)라면 이것은 모두 양의 실수로 나와야 한다. 여기에 대한 추가 설명은 \protect\hyperlink{Theoph}{14.5}를 참고한다.\index{상관행렬 / correlation matrix}\index{correlation matrix / 상관행렬}\index{\$COVARIANCE (\$COV, \$COVAR)}
...
NONMEM User's guide와 help file에는 \$COV를 다음과 같이 설명한다. (Beal \protect\hyperlink{ref-nonmem}{2018})\index{\$COVARIANCE (\$COV, \$COVAR)}

\begin{verbatim}
$COVARIANCE\index{\$COVARIANCE (\$COV, \$COVAR)}
...
\NormalTok{  Smat =}\StringTok{ }\KeywordTok{matrix}\NormalTok{(}\KeywordTok{rep}\NormalTok{(}\DecValTok{0}\NormalTok{, e}\OperatorTok{$}\NormalTok{nPara }\OperatorTok{*}\StringTok{ }\NormalTok{e}\OperatorTok{$}\NormalTok{nPara), }\DataTypeTok{nrow =}\NormalTok{ e}\OperatorTok{$}\NormalTok{nPara, }
  ...
  \ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\NormalTok{e}\OperatorTok{$}\NormalTok{nID) \{\textbackslash{}index\{ID\}}
\NormalTok{    e}\OperatorTok{$}\NormalTok{DATAi =}\StringTok{ }\NormalTok{e}\OperatorTok{$}\NormalTok{DataRef[[i]]\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{DATA\}}
...
\NormalTok{      gr =}\StringTok{ }\KeywordTok{grad}\NormalTok{(OiS1, e}\OperatorTok{$}\NormalTok{FinalPara)\textbackslash{}index\{S1\}}
\NormalTok{    \}}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{basic_files/figure-latex/theophconctime-1} \caption{Theophylline 경구 복용 후 농도-시간 곡선}\label{fig:theophconctime}
\end{figure}
...
\KeywordTok{colnames}\NormalTok{(DataAll) =}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"ID"}\NormalTok{, }\StringTok{"BWT"}\NormalTok{, }\StringTok{"DOSE"}\NormalTok{, }\StringTok{"TIME"}\NormalTok{, }\StringTok{"DV"}\NormalTok{)\textbackslash{}index\{ID\}\textbackslash{}index\{TIME\}}
\NormalTok{DataAll[,}\StringTok{"ID"}\NormalTok{] =}\StringTok{ }\KeywordTok{as.numeric}\NormalTok{(}\KeywordTok{as.character}\NormalTok{(DataAll[,}\StringTok{"ID"}\NormalTok{]))\textbackslash{}index\{ID\}}
...
\NormalTok{FGD1 =}\StringTok{ }\KeywordTok{deriv}\NormalTok{(}\OperatorTok{\textasciitilde{}}\NormalTok{DOSE}\OperatorTok{/}\NormalTok{(TH2}\OperatorTok{*}\KeywordTok{exp}\NormalTok{(ETA2))}\OperatorTok{*}\NormalTok{TH1}\OperatorTok{*}\KeywordTok{exp}\NormalTok{(ETA1)}\OperatorTok{/}\NormalTok{\textbackslash{}index\{D1\}}
\NormalTok{             (TH1}\OperatorTok{*}\KeywordTok{exp}\NormalTok{(ETA1) }\OperatorTok{{-}}\StringTok{ }\NormalTok{TH3}\OperatorTok{*}\KeywordTok{exp}\NormalTok{(ETA3))}\OperatorTok{*}
\NormalTok{			 (}\KeywordTok{exp}\NormalTok{(}\OperatorTok{{-}}\NormalTok{TH3}\OperatorTok{*}\KeywordTok{exp}\NormalTok{(ETA3)}\OperatorTok{*}\NormalTok{TIME)}\OperatorTok{{-}}\KeywordTok{exp}\NormalTok{(}\OperatorTok{{-}}\NormalTok{TH1}\OperatorTok{*}\KeywordTok{exp}\NormalTok{(ETA1)}\OperatorTok{*}\NormalTok{TIME)),\textbackslash{}index\{TIME\}}
             ...
             \DataTypeTok{function.arg=}\KeywordTok{c}\NormalTok{(}\StringTok{"TH1"}\NormalTok{, }\StringTok{"TH2"}\NormalTok{, }\StringTok{"TH3"}\NormalTok{, }\StringTok{"ETA1"}\NormalTok{, }\StringTok{"ETA2"}\NormalTok{, }\StringTok{"ETA3"}\NormalTok{, }
			                \StringTok{"DOSE"}\NormalTok{, }\StringTok{"TIME"}\NormalTok{),\textbackslash{}index\{TIME\}}
             \DataTypeTok{func=}\OtherTok{TRUE}\NormalTok{, }\DataTypeTok{hessian=}\OtherTok{FALSE}\NormalTok{)}

\NormalTok{H =}\StringTok{ }\KeywordTok{deriv}\NormalTok{(}\OperatorTok{\textasciitilde{}}\NormalTok{F }\OperatorTok{+}\StringTok{ }\NormalTok{F}\OperatorTok{*}\NormalTok{EPS1 }\OperatorTok{+}\StringTok{ }\NormalTok{EPS2, }\KeywordTok{c}\NormalTok{(}\StringTok{"EPS1"}\NormalTok{, }\StringTok{"EPS2"}\NormalTok{), \textbackslash{}index\{S1\}}
          \DataTypeTok{function.arg=}\KeywordTok{c}\NormalTok{(}\StringTok{"F"}\NormalTok{, }\StringTok{"EPS1"}\NormalTok{, }\StringTok{"EPS2"}\NormalTok{), \textbackslash{}index\{S1\}}
          \DataTypeTok{func=}\OtherTok{TRUE}\NormalTok{)}

\NormalTok{PRED1 =}\StringTok{ }\ControlFlowTok{function}\NormalTok{(THETA, ETA, DATAi)  }\CommentTok{\# for FO and FOCE\textbackslash{}index\{first{-}order conditional estimation method(FOCE) / 1차 조건부 추정방법(FOCE)\}\textbackslash{}index\{first{-}order method(FO) / 1차추정법\}\textbackslash{}index\{D1\}\textbackslash{}index\{FOCE\}\textbackslash{}index\{FO\}\textbackslash{}index\{PRED\}}
\NormalTok{\{}
\NormalTok{  FGDres =}\StringTok{ }\KeywordTok{FGD1}\NormalTok{(THETA[}\DecValTok{1}\NormalTok{], THETA[}\DecValTok{2}\NormalTok{], THETA[}\DecValTok{3}\NormalTok{], ETA[}\DecValTok{1}\NormalTok{], ETA[}\DecValTok{2}\NormalTok{], ETA[}\DecValTok{3}\NormalTok{], }\DataTypeTok{DOSE=}\DecValTok{320}\NormalTok{, \textbackslash{}index\{D1\}}
\NormalTok{                DATAi[,}\StringTok{"TIME"}\NormalTok{])\textbackslash{}index\{TIME\}}
...
\KeywordTok{InitStep}\NormalTok{(DataAll, }\DataTypeTok{THETAinit=}\NormalTok{THETAinit, }\DataTypeTok{OMinit=}\NormalTok{OMinit, }\DataTypeTok{SGinit=}\NormalTok{SGinit, }
         \DataTypeTok{LB=}\NormalTok{LB, }\DataTypeTok{UB=}\NormalTok{UB, }\DataTypeTok{Pred=}\NormalTok{PRED1, }\DataTypeTok{METHOD=}\StringTok{"ZERO"}\NormalTok{)\textbackslash{}index\{D1\}\textbackslash{}index\{PRED\}}
...
\CommentTok{\#\#       ID       ETA1         ETA2        ETA3\textbackslash{}index\{ID\}}
...
나머지 FOCE나 Laplacian에 대해서는 nmw 패키지의 도움말\footnote{\url{https://cran.r-project.org/web/packages/nmw/nmw.pdf}}을 보고 실행하면 알 수 있다. (Bae \protect\hyperlink{ref-R-nmw}{2018})\index{1차 조건부 추정방법(FOCE) / first-order conditional estimation method(FOCE)}\index{first-order conditional estimation method(FOCE) / 1차 조건부 추정방법(FOCE)}\index{first-order conditional estimation method(FOCE) / 1차 조건부 추정방법(FOCE)}\index{FOCE}\index{FO}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/15-fig-01} \caption{Example of EBE vs Covariates (Bonate \protect\hyperlink{ref-bonate}{2011})}\label{fig:ebe-cov}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.5\linewidth]{figures/15-fig-02} \caption{Concept of η shrinkage (Savic \protect\hyperlink{ref-uppsala}{2007})\index{shrinkage / 축소}\index{shrinkage / 축소}}\label{fig:eta-shrinkage}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.5\linewidth]{./figures/15-fig-03} \caption{Example 1 - Consequences of η shrinkage (Savic \protect\hyperlink{ref-uppsala}{2007})\index{shrinkage / 축소}\index{shrinkage / 축소}}\label{fig:eta-shrinkage-plot}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/15-fig-04} \caption{Example 2 - Consequences of η shrinkage (Savic \protect\hyperlink{ref-uppsala}{2007})\index{shrinkage / 축소}\index{shrinkage / 축소}}\label{fig:ex2-cons}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/15-fig-05} \caption{Example 3 - Consequences of η shrinkage (Savic \protect\hyperlink{ref-uppsala}{2007})\index{shrinkage / 축소}\index{shrinkage / 축소}}\label{fig:ex3-cons}
\end{figure}



\begin{figure}
\includegraphics[width=0.9\linewidth]{./assets/media-15/image8} \caption{Induced or hidden correlation according to average shrinkage (\%) (Savic \protect\hyperlink{ref-uppsala}{2007})\index{shrinkage / 축소}\index{shrinkage / 축소}}\label{fig:induced-corr}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/15-fig-06} \caption{Example 4 - Consequences of η shrinkage (Savic \protect\hyperlink{ref-uppsala}{2007})\index{shrinkage / 축소}\index{shrinkage / 축소}}\label{fig:ex4-cons}
\end{figure}
...
\begin{equation}
\epsilon_{sh} = 1 - SD(IWRES)\index{IWRES}\index{RES}\index{WRES}
\label{eq:eps-iwres}
\end{equation}
...
\begin{equation}
IWRES_{ij} = \frac{DV_{ij}- IPRED_{ij}}{SD(\epsilon)} \ \text{(If IPRED} \rightarrow \text{DV, IWRES} \rightarrow 0)\index{IPRED}\index{IWRES}\index{PRED}\index{RES}\index{WRES}
...
\begin{figure}
\includegraphics[width=0.5\linewidth]{figures/15-fig-07} \caption{Concept of ε shrinkage (Savic \protect\hyperlink{ref-uppsala}{2007})\index{shrinkage / 축소}\index{shrinkage / 축소}}\label{fig:concept-eps-shrinkage}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/15-fig-08} \caption{Perfect fit phenomenon (Savic \protect\hyperlink{ref-uppsala}{2007})}\label{fig:perfect-fit}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/15-fig-09} \caption{1-compartment model fitted to data simulated with a 2-compartment model (Savic \protect\hyperlink{ref-uppsala}{2007})}\label{fig:1comp-2comp}
\end{figure}



\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/15-fig-10} \caption{Linear Emax model fitted to data simulated with a sigmoidal Emax model (Savic \protect\hyperlink{ref-uppsala}{2007})}\label{fig:linear-sigmoidal}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/15-fig-11} \caption{Low power of IWRES to diagnose Residual error misspecification (Savic \protect\hyperlink{ref-uppsala}{2007})\index{IWRES}\index{RES}\index{WRES}}\label{fig:low-IWRES}
\end{figure}
...
\begin{verbatim}
## Error: <text>:1:79: unexpected input
## 1: readxl::read_excel("data-raw/fig-tab.xlsx", sheet="16-1", range="B2:D10") %>% \
##                                                                                   ^
\end{verbatim}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/16-fig-01} \caption{붓스트랩을 사용하여 여러 파라미터의 중앙값과 95\% 신뢰구간을 계산하는 방법에 대한 모식도\index{붓스트랩 / bootstrapping}\index{bootstrapping / 붓스트랩}}\label{fig:boot95}
\end{figure}



\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/16-fig-02} \caption{붓스트랩에 의해 얻어진 결과를 정리하는 방법의 예. 중앙값(95\% 신뢰구간)을 열로 정리하였고 1000개의 재추출된 데이터셋이 사용되었음을 알 수 있음\index{붓스트랩 / bootstrapping}\index{bootstrapping / 붓스트랩}}\label{fig:bootstrap-table}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/16-fig-03} \caption{사후 예측 점검의 예. 시뮬레이션된 Cmax의 분포에서 관찰값(검정색 실선)의 위치를 파악할 수 있음\index{시뮬레이션 / simulation}\index{simulation / 시뮬레이션}}\label{fig:ppc}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/16-fig-04} \caption{시각적 예측 점검 과정의 대표적인 scatter VPC 의 예 (Harling \protect\hyperlink{ref-vpcnpc}{2018}). 점선은 시뮬레이션 값들의 5, 95 퍼센타일, 실선은 중앙값을 나타냄\index{시각적 예측 점검(VPC) / visual predictive check(VPC)}\index{시뮬레이션 / simulation}\index{simulation / 시뮬레이션}\index{visual predictive check(VPC) / 시각적 예측 점검(VPC)}}\label{fig:scatter-vpc}
\end{figure}



\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/16-fig-05} \caption{신뢰구간 VPC의 예 (Confidence interval VPC). 시뮬레이션된 예측값의 5번째 백분위수와 95번째 백분위수에 대한 95\% 신뢰구간이 파란색 띠로 나타나 있고, 붉은색 점선은 관찰값의 5번쨰, 95번째 백분위수를 나타내며 붉은색 실선은 관찰값의 50번째 백분위수임\index{시뮬레이션 / simulation}\index{simulation / 시뮬레이션}}\label{fig:ci-vpc}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/16-fig-06} \caption{관찰일(DAY)에 따라 층화(stratification)가 이루어진 VPC.}\label{fig:strat-vpc}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/16-fig-07} \caption{수치로 모델의 예측성능을 평가하는 수치적 예측 점검 과정(NPC)\index{수치적 예측 점검(NPC) / numerical predictive check(NPC)}\index{numerical predictive check(NPC) / 수치적 예측 점검(NPC)}}\label{fig:npc}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/17-fig-01} \caption{용량-노출, 노출-약효의 상관관계}\label{fig:unnamed-chunk-3}
\end{figure}
...
\begin{verbatim}
## Error: <text>:15:76: unexpected input
## 14: 2)비척도(Ratio scale): 체중, 혈압, 농도 등 (절대적 0의 값 있음)',
## 15: 'IPRED나 PRED 가 DV 값 자체와 얼마나 비슷한지가 중요(가장 흔히 쓰는 방식)',\
##                                                                                ^
\end{verbatim}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/17-fig-02} \caption{PD의 시간에 따른 변화의 사례 (Primary Psychiatry \protect\hyperlink{ref-dementia}{2013})}\label{fig:pd-time-course}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/17-fig-03} \caption{혈장약물농도와 약효의 시간적 관계 (Primary Psychiatry \protect\hyperlink{ref-dementia}{2013})}\label{fig:conc-hysteresis}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/18-fig-01} \caption{PK-PD의 연결고리}\label{fig:pkpd-link}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/18-fig-02} \caption{Clopidogrel의 PK-PD 모델 구조 (Lee et al. \protect\hyperlink{ref-lee2012population}{2012})}\label{fig:clopidogrel}
\end{figure}
...
\centering
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{2cm}>{\raggedright\arraybackslash}p{4cm}>{\raggedright\arraybackslash}p{1cm}>{\raggedright\arraybackslash}p{2cm}>{\raggedright\arraybackslash}p{4cm}}
\toprule
분석 & PK-PD
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/18-fig-03} \caption{PK-PD 연결 방법과 그에 관련된 정보의 수준}\label{fig:level-info}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.5\linewidth]{./assets/media-18/image5} \caption{SIM 방법의 데이터셋의 예}\label{fig:SIM-dataset}
\end{figure}
...
\KeywordTok{IF}\NormalTok{ (CMT.LE}\FloatTok{.1}\NormalTok{) Y =}\StringTok{ }\NormalTok{Y1 }\OperatorTok{+}\StringTok{ }\KeywordTok{ERR}\NormalTok{(}\DecValTok{1}\NormalTok{) \textbackslash{}index\{CMT\}}
\KeywordTok{IF}\NormalTok{ (CMT.EQ}\FloatTok{.2}\NormalTok{) Y =}\StringTok{ }\NormalTok{Y2 }\OperatorTok{+}\StringTok{ }\KeywordTok{ERR}\NormalTok{(}\DecValTok{2}\NormalTok{)\textbackslash{}index\{CMT\}}
...
\OperatorTok{$}\NormalTok{INPUT ID TIME AMT DV MDV CMT \textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{INPUT\}\textbackslash{}index\{AMT\}\textbackslash{}index\{CMT\}\textbackslash{}index\{ID\}\textbackslash{}index\{MDV\}\textbackslash{}index\{TIME\}}
\OperatorTok{$}\NormalTok{DATA DATAFILENAME.csv IGNORE=}\ErrorTok{@}\StringTok{ }\NormalTok{\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{DATA\}\textbackslash{}index\{IGNORE\}}
\OperatorTok{$}\NormalTok{SUBROUTIN ADVAN6 TOL=}\DecValTok{3}\NormalTok{ \textbackslash{}index\{ADVAN6\}\textbackslash{}index\{TOL\}}
\OperatorTok{$}\NormalTok{MODEL \textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{MODEL\}}
\KeywordTok{COMP}\NormalTok{ (CENT, DEFDOSE, DEFOBS) \textbackslash{}index\{COMP\}\textbackslash{}index\{DEFDOS\}\textbackslash{}index\{DEFOBS\}\textbackslash{}index\{FO\}}
\KeywordTok{COMP}\NormalTok{ (PD)\textbackslash{}index\{COMP\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\OperatorTok{$}\NormalTok{ERROR \textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{ERROR\}}
\KeywordTok{IF}\NormalTok{ (CMT.EQ}\FloatTok{.1}\NormalTok{) THEN \textbackslash{}index\{CMT\}}
\NormalTok{  IPRED =}\StringTok{ }\NormalTok{CP \textbackslash{}index\{IPRED\}\textbackslash{}index\{PRED\}}
\NormalTok{  W =}\StringTok{ }\KeywordTok{SQRT}\NormalTok{(}\KeywordTok{THETA}\NormalTok{(}\DecValTok{10}\NormalTok{)}\OperatorTok{**}\DecValTok{2} \OperatorTok{+}\StringTok{ }\KeywordTok{THETA}\NormalTok{(}\DecValTok{11}\NormalTok{)}\OperatorTok{**}\DecValTok{2} \OperatorTok{*}\StringTok{ }\NormalTok{IPRED}\OperatorTok{**}\DecValTok{2}\NormalTok{) \textbackslash{}index\{IPRED\}\textbackslash{}index\{PRED\}}
\NormalTok{  IRES =}\StringTok{ }\NormalTok{DV }\OperatorTok{{-}}\StringTok{ }\NormalTok{IPRED \textbackslash{}index\{IPRED\}\textbackslash{}index\{IRES\}\textbackslash{}index\{PRED\}\textbackslash{}index\{RES\}}
\NormalTok{  IWRES =}\StringTok{ }\NormalTok{IRES }\OperatorTok{/}\StringTok{ }\NormalTok{W \textbackslash{}index\{IRES\}\textbackslash{}index\{IWRES\}\textbackslash{}index\{RES\}\textbackslash{}index\{WRES\}}
\NormalTok{  Y =}\StringTok{ }\NormalTok{IPRED }\OperatorTok{+}\StringTok{ }\NormalTok{W }\OperatorTok{*}\StringTok{ }\KeywordTok{EPS}\NormalTok{(}\DecValTok{1}\NormalTok{) \textbackslash{}index\{IPRED\}\textbackslash{}index\{PRED\}}
\KeywordTok{IF}\NormalTok{ (CMT.EQ}\FloatTok{.2}\NormalTok{) THEN \textbackslash{}index\{CMT\}}
\NormalTok{  IPRED =}\StringTok{ }\NormalTok{EMAX}\OperatorTok{*}\NormalTok{CP}\OperatorTok{/}\NormalTok{(EC50}\OperatorTok{+}\NormalTok{CP) \textbackslash{}index\{IPRED\}\textbackslash{}index\{PRED\}}
\NormalTok{  W =}\StringTok{ }\KeywordTok{THETA}\NormalTok{(}\DecValTok{12}\NormalTok{) }
\NormalTok{  IRES =}\StringTok{ }\NormalTok{DV }\OperatorTok{{-}}\StringTok{ }\NormalTok{IPRED \textbackslash{}index\{IPRED\}\textbackslash{}index\{IRES\}\textbackslash{}index\{PRED\}\textbackslash{}index\{RES\}}
\NormalTok{  IWRES =}\StringTok{ }\NormalTok{IRES }\OperatorTok{/}\StringTok{ }\NormalTok{W \textbackslash{}index\{IRES\}\textbackslash{}index\{IWRES\}\textbackslash{}index\{RES\}\textbackslash{}index\{WRES\}}
\NormalTok{  Y =}\StringTok{ }\NormalTok{IPRED }\OperatorTok{+}\StringTok{ }\NormalTok{W }\OperatorTok{*}\StringTok{ }\KeywordTok{EPS}\NormalTok{(}\DecValTok{2}\NormalTok{) \textbackslash{}index\{IPRED\}\textbackslash{}index\{PRED\}}
\NormalTok{ENDIF}
\end{Highlighting}
\end{Shaded}
...
\begin{figure}
\includegraphics[width=0.5\linewidth]{./assets/media-18/image6} \caption{PPP\&D 방법의 데이터셋의 예}\label{fig:pppd}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./assets/media-18/image7} \caption{Example of dataset for PPP}\label{fig:PPP-dataset}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.5\linewidth]{./assets/media-18/image8} \caption{IPP 방법의 데이터셋의 예}\label{fig:ipp-dataset-ex}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.5\linewidth]{./assets/media-18/image9} \caption{SIM 방법과 PPP\&D 방법을 쓸 때의 PK-PD 데이터셋의 예}\label{fig:sim-pppd-dataset}
\end{figure}
...
\OperatorTok{$}\NormalTok{DATA SIM\_S.csv IGNORE=}\ErrorTok{@}\NormalTok{\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{DATA\}\textbackslash{}index\{IGNORE\}}
\OperatorTok{$}\NormalTok{INPUT ID TIME AMT DMT DV MDV SEX AGE WT HT\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{INPUT\}\textbackslash{}index\{AMT\}\textbackslash{}index\{ID\}\textbackslash{}index\{MDV\}\textbackslash{}index\{TIME\}}

\OperatorTok{$}\NormalTok{SUBROUTINE ADVAN6 TOL=}\DecValTok{3}\NormalTok{\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{SUBROUTINE\}\textbackslash{}index\{ADVAN6\}\textbackslash{}index\{TOL\}}

\OperatorTok{$}\NormalTok{MODEL NCOMP=}\DecValTok{4}\NormalTok{\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{MODEL\}\textbackslash{}index\{COMP\}}
  \KeywordTok{COMP}\NormalTok{(DEPOT, DEFDOSE)\textbackslash{}index\{COMP\}\textbackslash{}index\{DEFDOS\}}
  \KeywordTok{COMP}\NormalTok{(CENT)\textbackslash{}index\{COMP\}}
  \KeywordTok{COMP}\NormalTok{(PERI)\textbackslash{}index\{COMP\}}
  \KeywordTok{COMP}\NormalTok{(EFFE, DEFOBS)\textbackslash{}index\{COMP\}\textbackslash{}index\{DEFOBS\}\textbackslash{}index\{FO\}}

\OperatorTok{$}\NormalTok{PK\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{PK\}}
\NormalTok{;}\OperatorTok{{-}{-}{-}{-}}\StringTok{ }\NormalTok{PK FIXED EFFECT }\OperatorTok{{-}{-}{-}{-}}\NormalTok{\textbackslash{}index\{FIXED\}}
\NormalTok{  TVCL =}\StringTok{ }\KeywordTok{THETA}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{  TVV2 =}\StringTok{ }\KeywordTok{THETA}\NormalTok{(}\DecValTok{2}\NormalTok{)}
\NormalTok{  TVV3 =}\StringTok{ }\KeywordTok{THETA}\NormalTok{(}\DecValTok{3}\NormalTok{)}
\NormalTok{  TVQ  =}\StringTok{ }\KeywordTok{THETA}\NormalTok{(}\DecValTok{4}\NormalTok{)}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./assets/media-18/image13} \caption{PPP 방법 쓸 때의 PK-PD 데이터셋의 기본 구조}\label{fig:ppp-pkpd-dataset}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./assets/media-18/image14} \caption{IPP 방법 쓸 때의 PK-PD 데이터셋의 기본 구조}\label{fig:ipp-dataset}
\end{figure}
...
\begin{Highlighting}[]
\OperatorTok{$}\NormalTok{PROB HO\_PO\_2comp}
\OperatorTok{$}\NormalTok{DATA IPP\_S.csv IGNORE=}\ErrorTok{@}\NormalTok{\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{DATA\}\textbackslash{}index\{IGNORE\}}
\OperatorTok{$}\NormalTok{INPUT ID TIME AMT CMT DV MDV SEX AGE WT HT ICL IV2 IV3 IKA\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{INPUT\}\textbackslash{}index\{AMT\}\textbackslash{}index\{CMT\}\textbackslash{}index\{ID\}\textbackslash{}index\{MDV\}\textbackslash{}index\{TIME\}}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/18-fig-11} \caption{순차적 방법과 SIM 방법 쓸 때의 절약되는 시간 비교 (N=200) (Zhang, Beal, and Sheiner \protect\hyperlink{ref-zhang1}{2003}\protect\hyperlink{ref-zhang1}{a})}\label{fig:time-comparison}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/18-fig-12} \caption{Average absolute (A) and signed error (B) of PD parameter estimates (Zhang, Beal, and Sheiner \protect\hyperlink{ref-zhang1}{2003}\protect\hyperlink{ref-zhang1}{a})}\label{fig:absolute-signed}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.5\linewidth]{figures/19-fig-01} \caption{선형 PD 모델 (dots: 관찰값, line: 예측값)}\label{fig:linear-pd}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./assets/media-19/image2} \caption{로그-선형 모델 (dots: 관찰값, line: 선형로그모델의 예측값, dotted line: Emax모델의 예측값)}\label{fig:log-linear-model}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./assets/media-19/image5} \caption{Emax model (좌)선형농도, (우)로그농도}\label{fig:emax-model}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/19-fig-04} \caption{E\textsubscript{max} 모델의 비교 (좌) \(E \approx E_{0} + \frac{\ E_{\max}\lbrack C\rbrack}{EC_{50} + \lbrack C\rbrack}\) (우) \(E \approx E_{0} \cdot (1 + \frac{\ E_{\max}\left\lbrack C \right\rbrack}{EC_{50} + \left\lbrack C \right\rbrack})\)}\label{fig:emax-comparison}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{./assets/media-19/image7} \caption{길항제의 E\textsubscript{max} 모델}\label{fig:anta-emax}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/19-fig-06} \caption{Sigmoid E\textsubscript{max} model. H에 따른 E\textsubscript{max} model의 형태 변화}\label{fig:sigmoid-emax}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.5\linewidth]{figures/19-fig-sigmoid-emax} \caption{Sigmoid E\textsubscript{max} model: 로그농도-반응 그래프}\label{fig:sigmoid-emax-block2}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/19-fig-07} \caption{일반적인 I\textsubscript{max} model(빨강)과 sigmoid I\textsubscript{max} model(파랑)의 비교}\label{fig:sigmoid-imax}
\end{figure}
...
\begin{equation}
[C] + [R] \xrightarrow[k_{-1}]{k_1} [CR] + [C] \xrightarrow[k_{-2}]{k_2} [C2R]
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/19-fig-08} \caption{K\textsubscript{2}의 변화에 따른 약물농도-약효와의 관계}\label{fig:K2}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/20-fig-01} \caption{PK와 PD의 관련성 (Gabrielsson \protect\hyperlink{ref-gabrielsson}{2006})}\label{fig:pkpd-relationship}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/20-fig-02} \caption{히스테레시스(반시계방향 히스테레시스)와 프로테레시스(시계방향 히스테레시스)}\label{fig:hys-proteresis}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/20-fig-03} \caption{효과구획의 도식 (C: plasma concentration (Actually observed), Ce: effect compartment concentration (NOT observed, produced), R: response (Actually observed) (Gabrielsson \protect\hyperlink{ref-gabrielsson}{2006})}\label{fig:effect-comp}
\end{figure}
...
\begin{example}
\protect\hypertarget{exm:effect-model-code}{}{\label{exm:effect-model-code} }효과구획 모델 코드: PREDPP Linear Model (ADVAN3, Two compartment) (Beal \protect\hyperlink{ref-nonmem}{2018})\index{ADVAN3}\index{PRED}\index{PREDPP}
\end{example}

\begin{Shaded}
\begin{Highlighting}[]
\OperatorTok{$}\NormalTok{SUBROUTINES ADVAN3\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{SUBROUTINE\}\textbackslash{}index\{ADVAN3\}}
\OperatorTok{$}\NormalTok{PK\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{PK\}}
\NormalTok{  K   =}\StringTok{ }\KeywordTok{THETA}\NormalTok{(}\DecValTok{1}\NormalTok{)}\OperatorTok{*}\KeywordTok{EXP}\NormalTok{(}\KeywordTok{ETA}\NormalTok{(}\DecValTok{1}\NormalTok{))        ; TRIVIAL LOSS TO EFFECT CMPT\textbackslash{}index\{SS\}}
\NormalTok{  K12 =}\StringTok{ }\FloatTok{.001}\OperatorTok{*}\NormalTok{K                      ; KEO}
\NormalTok{  K21 =}\StringTok{ }\KeywordTok{THETA}\NormalTok{(}\DecValTok{2}\NormalTok{)}\OperatorTok{*}\KeywordTok{EXP}\NormalTok{(}\KeywordTok{ETA}\NormalTok{(}\DecValTok{2}\NormalTok{))        ; V1 FOR DRUG\textbackslash{}index\{FO\}}
\NormalTok{  S1  =}\StringTok{ }\KeywordTok{THETA}\NormalTok{(}\DecValTok{3}\NormalTok{)}\OperatorTok{*}\KeywordTok{EXP}\NormalTok{(}\KeywordTok{ETA}\NormalTok{(}\DecValTok{3}\NormalTok{))\textbackslash{}index\{S1\}}
...
\NormalTok{  S2  =}\StringTok{ }\NormalTok{S1}\OperatorTok{*}\NormalTok{K12}\OperatorTok{/}\NormalTok{K21                  ; SO THAT CESS =}\StringTok{ }\NormalTok{CPSS\textbackslash{}index\{S1\}\textbackslash{}index\{SS\}}
\OperatorTok{$}\NormalTok{ERROR\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{ERROR\}}
  \KeywordTok{IF}\NormalTok{ (CMT.EQ}\FloatTok{.2}\NormalTok{) THEN\textbackslash{}index\{CMT\}}
\NormalTok{  Y   =}\StringTok{ }\NormalTok{EMAX}\OperatorTok{*}\NormalTok{F}\OperatorTok{/}\NormalTok{(C50}\OperatorTok{+}\NormalTok{F)}\OperatorTok{*}\KeywordTok{EXP}\NormalTok{(}\KeywordTok{ERR}\NormalTok{(}\DecValTok{2}\NormalTok{))  ; DV =}\StringTok{ }\NormalTok{EFFECT IF CMT=}\DecValTok{2}\NormalTok{\textbackslash{}index\{CMT\}}
\NormalTok{  ELSE}
\NormalTok{  Y   =}\StringTok{ }\NormalTok{F}\OperatorTok{*}\KeywordTok{EXP}\NormalTok{(}\KeywordTok{ERR}\NormalTok{(}\DecValTok{1}\NormalTok{))               ; DV =}\StringTok{ }\NormalTok{CP IF CMT=}\DecValTok{1}\NormalTok{ (OR }\DecValTok{0}\NormalTok{)\textbackslash{}index\{CMT\}}
\NormalTok{  ENDIF}
\end{Highlighting}
\end{Shaded}
...
\begin{example}
\protect\hypertarget{exm:effect-comp-advan6}{}{\label{exm:effect-comp-advan6} }효과구획 모델 코드: Using General Nonlinear model (ADVAN6) (Beal \protect\hyperlink{ref-nonmem}{2018})\index{ADVAN6}
\end{example}

\begin{Shaded}
\begin{Highlighting}[]
\OperatorTok{$}\NormalTok{SUBROUTINES ADVAN6 TOL=}\DecValTok{5}\NormalTok{\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{SUBROUTINE\}\textbackslash{}index\{ADVAN6\}\textbackslash{}index\{TOL\}}
\OperatorTok{$}\NormalTok{MODEL\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{MODEL\}}
\NormalTok{  COMP=(CENTRAL,DEFDOSE,DEFOBS)\textbackslash{}index\{COMP\}\textbackslash{}index\{DEFDOS\}\textbackslash{}index\{DEFOBS\}\textbackslash{}index\{FO\}}
\NormalTok{  COMP=EFFECT\textbackslash{}index\{COMP\}}
\OperatorTok{$}\NormalTok{PK\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{PK\}}
...
\NormalTok{  S1  =}\StringTok{ }\NormalTok{V1 \textbackslash{}index\{S1\}}
...
\OperatorTok{$}\NormalTok{DES\textbackslash{}index\{\textbackslash{}}\OperatorTok{$}\NormalTok{DES\}}
  \KeywordTok{DADT}\NormalTok{(}\DecValTok{1}\NormalTok{)=}\OperatorTok{{-}}\NormalTok{K10}\OperatorTok{*}\KeywordTok{A}\NormalTok{(}\DecValTok{1}\NormalTok{)                 ; NOTE NO LOSS TO }\KeywordTok{A}\NormalTok{(}\DecValTok{2}\NormalTok{)\textbackslash{}index\{SS\}}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/20-fig-04} \caption{직접효과와 간접효과의 모식도 (Gabrielsson \protect\hyperlink{ref-gabrielsson}{2006})\index{직접 효과 모델 / direct-effect model}\index{direct-effect model / 직접 효과 모델}}\label{fig:direct-indirect}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/20-fig-05} \caption{Turnover 모델(간접효과 모델)의 네 가지 (Sharma and Jusko \protect\hyperlink{ref-sharma1998characteristics}{1998})}\label{fig:turnover-4}
\end{figure}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{C1=}\KeywordTok{A}\NormalTok{(}\DecValTok{1}\NormalTok{)}\OperatorTok{/}\NormalTok{S1\textbackslash{}index\{S1\}}
...
\begin{figure}
\includegraphics[width=0.5\linewidth]{figures/20-fig-06} \caption{Inhibition on production (Sharma and Jusko \protect\hyperlink{ref-sharma1998characteristics}{1998})}\label{fig:model-i}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.5\linewidth]{figures/20-fig-07} \caption{Inhibition on loss (Sharma and Jusko \protect\hyperlink{ref-sharma1998characteristics}{1998})}\label{fig:model-ii}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.5\linewidth]{figures/20-fig-08} \caption{Stimulation on production (Sharma and Jusko \protect\hyperlink{ref-sharma1998characteristics}{1998})}\label{fig:model-iii}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.5\linewidth]{figures/20-fig-09} \caption{Stimulation on loss (Sharma and Jusko \protect\hyperlink{ref-sharma1998characteristics}{1998})}\label{fig:model-iv}
\end{figure}
