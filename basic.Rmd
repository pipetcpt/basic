...
#author: "대표저자 임동석"
#"임동석, 이소진, 배수현, 전상일, 홍태곤, 한승훈, 김정렬, 배균섭, 한성필"
...
`r if (knitr::is_html_output()) ' [<img src="images/cover.jpg" style="max-width:30%;min-width:80px;float:right;margin: 10px 10px 5px 5px" alt="표지" />](images/cover.jpg) '`
...
```{r tab}
tab <- readxl::read_excel('assets/02-table-01.xlsx') %>% 
...
이 장은 다음 문헌을 참고하였다. [@kelly; @nonmem]
...
$INPUT ID TIME AMT RATE DUR DV MDV WT AGE SEX HT RF ALB CLCR\index{\$INPUT}\index{AMT}\index{ID}\index{MDV}\index{RATE}\index{TIME}
$DATA test.csv IGNORE=@\index{\$DATA}\index{IGNORE}
$SUBR ADVAN1 TRANS2\index{ADVAN1}\index{TRANS}\index{TRANS2}
$PK\index{\$PK}
  V  = THETA(1)*EXP(ETA(1))
  CL = THETA(2)*EXP(ETA(2))
  S1 = V\index{S1}
$ERROR\index{\$ERROR}
  IPRED = F\index{IPRED}\index{PRED}
  W     = SQRT(THETA(3)**2 + THETA(4)**2 * IPRED**2)\index{IPRED}\index{PRED}
  IRES  = DV - IPRED\index{IPRED}\index{IRES}\index{PRED}\index{RES}
  IWRES = IRES / W\index{IRES}\index{IWRES}\index{RES}\index{WRES}
  Y     = IPRED + W * EPS(1)\index{IPRED}\index{PRED}
$THETA\index{\$THETA}
  (0, 400)
  (0, 100)
  0.0001 FIX
...
$SIGMA\index{\$SIGMA}
  1 FIX
$ESTIMATION NOABORT MAXEVAL=9999 METHOD=1 INTER PRINT=5\index{\$ESTIMATION (\$EST)}\index{MAXEVAL}\index{NOABORT}
$TABLE ID TIME AMT RATE MDV DV IPRED IRES CWRES \index{\$TABLE}\index{AMT}\index{CWRES}\index{ID}\index{IPRED}\index{IRES}\index{MDV}\index{PRED}\index{RATE}\index{RES}\index{TIME}\index{WRES}
       ONEHEADER NOPRINT FILE=sdtab1001\index{FILE=}\index{NOPRINT}\index{ONEHEADER}
$TABLE ID TIME AMT DV CL V ETA1 ETA2 \index{\$TABLE}\index{AMT}\index{ID}\index{TIME}
       ONEHEADER NOPRINT FILE=patab1001\index{FILE=}\index{NOPRINT}\index{ONEHEADER}
$TABLE ID RF SEX \index{\$TABLE}\index{ID}
       ONEHEADER NOPRINT FILE=catab1001\index{FILE=}\index{NOPRINT}\index{ONEHEADER}
$TABLE ID WT AGE SEX HT ALB CLCR \index{\$TABLE}\index{ID}
       ONEHEADER NOPRINT FILE=cotab1001\index{FILE=}\index{NOPRINT}\index{ONEHEADER}
```
...
```r
$DATA test.csv IGNORE = # IGNORE=(SEX.EQ.1)\index{\$DATA}\index{IGNORE}
```

또는

```r
$DATA test.csv IGNORE = # IGNORE=(WEIGHT.LE.40)\index{\$DATA}\index{IGNORE}
```

또는

```r
$DATA test.csv IGNORE = # IGNORE=(WEIGHT.LE.40, SEX.EQ.1)\index{\$DATA}\index{IGNORE}
```
...
2구획 경구 약동학 모델의 \$PK 레코드\index{\$PK}
```

```r
$SUBROUTINE ADVAN4 TRANS4\index{\$SUBROUTINE}\index{ADVAN4}\index{TRANS}
...
\begin{equation}
\frac{Amount\ (mg)}{Vd\ (L)} \neq \ DV\ (μg/L)
(\#eq:scalingparam)\index{scaling parameter / 척도 파라미터}\index{scaling parameter / 척도 파라미터}
\end{equation}

위의 식 \@red(eq:scalingparam)에서 좌변에 1000을 곱하면 단위가 DV와 통일된다. `S2=V2/1000`이며, 이를 \$PK에 코드 \@ref(exm:two-comp-pk)처럼 기술한다. 동일한 모델을 분석할 경우, 사용자가 정의한 일반 ADVAN을 사용한 모델보다 특수 ADVAN 모델의 실행시간이 더 짧다.\index{특수 ADVAN / specific ADVANs}\index{scaling parameter / 척도 파라미터}\index{specific ADVANs / 특수 ADVAN}\index{scaling parameter / 척도 파라미터}\index{specific ADVANs / 특수 ADVAN}\index{\$PK}

```{r advan-param}
txt <- "Parameters*, Descriptions
S1, Scale for the first compartment\index{S1}
F1, Bioavailability for the first compartment\index{F1}
R1, Rate for the first compartment\index{R1}
D1, Duration for the first compartment\index{D1}
ALAG1, Absorption lag parameter"\index{ALAG}
readr::read_csv(txt) %>% 
...
```r
$THETA (0, 100) (0, 30) (0, 0.7)\index{\$THETA}
```
...
```r
$OMEGA 0.16 0.25 0.16\index{\$OMEGA}
```
...
```r
$OMEGA BLOCK(2)\index{\$OMEGA}\index{\$OMEGA BLOCK}
0.16
0.01 0.25
$OMEGA 0.16\index{\$OMEGA}
```
...
IPRED = F \index{IPRED}\index{PRED}
IRES = DV-IPRED \index{IPRED}\index{IRES}\index{PRED}\index{RES}
IWRES = IRES/IPRED \index{IPRED}\index{IRES}\index{IWRES}\index{PRED}\index{RES}\index{WRES}
Y = F*(1+EPS(1)) +EPS(2)
```
...
$ERROR \index{\$ERROR}
IPRED = F \index{IPRED}\index{PRED}
W = SQRT(THETA(4)**2 + THETA(5)**2 *IPRED**2) \index{IPRED}\index{PRED}
IRES = DV-IPRED \index{IPRED}\index{IRES}\index{PRED}\index{RES}
IWRES = IRES/W \index{IRES}\index{IWRES}\index{RES}\index{WRES}
Y = IPRED + W * EPS(1)\index{IPRED}\index{PRED}
...
$THETA (0, 100) (0, 30) (0, 0.7) 10 0.2\index{\$THETA}
...
$SIGMA 1 FIX\index{\$SIGMA}
```
...
$ESTIMATION은 파라미터 추정을 위한 추정방법을 선택하고 추정결과를 어떻게 출력할 것인지를 명시하는 레코드이다.\index{\$ESTIMATION (\$EST)} 추정방법에는 1차추정법(first order, FO), 1차 조건부 추정법 (first-order conditional estimation, FOCE), 라플라시안 추정법 등을 비롯하여 다양한 방법들이 있다. 추정방법에 대한 이론적인 설명은 [9장](#estimation-methods)에서 자세히 다루기로 하고 여기서는 제어구문에 사용하는 기본적인 코드와 용어에 대해서 설명한다.
...
```r
$SIMULATION (seed) ONLYSIM NSUB=1000\index{\$SIMULATION (\$SIM)}\index{NSUB}\index{ONLYSIM}
```
...
```r
$COVARIANCE PRINT=E UNCONTIONAL MATRIX=S\index{\$COVARIANCE (\$COV, \$COVAR)}\index{CONT}\index{PRINT=E}
```
...
2구획 경구 약동학 모델의 제어구문의 예. ADVAN4\index{ADVAN4}
```
...
$PK\index{\$PK}
  CL    = THATA(1) * EXP(ETA(1))
  V2    = THATA(2) * EXP(ETA(2))
  V3    = THATA(3) * EXP(ETA(3))
  Q     = THATA(4) * EXP(ETA(4))
  KA    = THATA(5) * EXP(ETA(5))
...
$ERROR\index{\$ERROR}
  IPRED = F\index{IPRED}\index{PRED}
  W     = SQRT(THETA(6))**2 + THETA(7)**2 * IPRED**2)\index{IPRED}\index{PRED}
  IRES  = DV - IPRED\index{IPRED}\index{IRES}\index{PRED}\index{RES}
  IWRES = IRES / W\index{IRES}\index{IWRES}\index{RES}\index{WRES}
  Y     = IPRED + W * EPS(1) \index{IPRED}\index{PRED}
...
2구획 경구 약동학 모델의 제어구문의 예. ADVAN5 \index{ADVAN5}
```

```r
$SUBROUTINE ADVAN5\index{\$SUBROUTINE}\index{ADVAN5}

$MODEL\index{\$MODEL}
  COMP(DEPOT, DEFDOSE)\index{COMP}\index{DEFDOS}
  COMP(CENT, DEFOBS)\index{COMP}\index{DEFOBS}\index{FO}
  COMP(PERI)\index{COMP}
...
2구획 경구 약동학 모델의 제어구문의 예. ADVAN6 \index{ADVAN6}
```

```r
$SUBROUTINE ADVAN6 TOL=4\index{\$SUBROUTINE}\index{ADVAN6}\index{TOL}

$MODEL\index{\$MODEL}
  COMP(DEPOT, DEFDOSE)\index{COMP}\index{DEFDOS}
  COMP(CENTRAL, DEFOBS)\index{COMP}\index{DEFOBS}\index{FO}
  COMP(PERIPH)\index{COMP}
...
```{r model-parent-metabolite, fig.cap="(ref:model-parent-metabolite)"}
...
```r
$PROBLEM Model of Parent and Metabolite Concentrations\index{\$PROBLEM}
$INPUT ID TIME DV AMT EVID MDV CMT\index{\$INPUT}\index{AMT}\index{CMT}\index{EVID}\index{ID}\index{MDV}\index{TIME}
$DATA filename\index{\$DATA}
$SUBROUTINE ADVAN6 TOL=4\index{\$SUBROUTINE}\index{ADVAN6}\index{TOL}
$MODEL\index{\$MODEL}
  COMP (DEPOT, DEFDOS)\index{COMP}\index{DEFDOS}
  COMP (CENTPRNT, DEFOBS)\index{COMP}\index{DEFOBS}\index{FO}
  COMP (CENTMETB)\index{COMP}
$PK\index{\$PK}
...
$ERROR\index{\$ERROR}
  IF (CMT.EQ.2) TYPE=0 ;Parent concentration\index{CMT}
  IF (CMT.EQ.3) TYPE=1 ;Metabolite concentration\index{CMT}
Y = F*EPS(1) * (1-TYPE)
```
...
\$PRED를 이용한 간단한 선형 모델\index{\$PRED}\index{PRED}
```

```r
$PROB QTc, PRED\index{PRED}
$INPUT ID=DROP DQTC=DV CMAX\index{\$INPUT}\index{DROP}\index{ID}
$DATA data.csv IGNORE=C \index{\$DATA}\index{IGNORE}
; Since the header row starts with a C,
; that row is dropped
$PRED\index{\$PRED}\index{PRED}
  INT = THETA(1)       ; Intercept
  SLP = THETA(2)       ; Slope
  EFF = SLP*CMAX + INT ; Drug effect model- linear
  Y   = EFF + ETA(1)   ; Residual error model - additive
$EST PRINT=5 MAX=9999 SIG=3\index{\$ESTIMATION (\$EST)}
$THETA\index{\$THETA}
  0.1 ; Intercept
  0.5 ; Slope
$OMEGA\index{\$OMEGA}
...
```{r 2comp-ex, out.width="50%", fig.cap="(ref:2comp-ex)"}
...
tibble::tribble(
  ...
  "ADVAN1", "TRANS1", "K",
  "ADVAN1", "TRANS2", "CL, V", # bold: \\textbf{TRANS2}\index{ADVAN1}\index{TRANS}\index{TRANS2}
  "ADVAN2", "TRANS1", "K, KA",
  "ADVAN2", "TRANS2", "CL, V, KA", # bold: \\textbf{TRANS2}\index{ADVAN2}\index{TRANS}\index{TRANS2}
  "ADVAN3", "TRANS1", "K, K12, K21",
  "ADVAN3", "TRANS3", "CL, V, Q, VSS",
  "ADVAN3", "TRANS4", "CL, V1, Q, V2", # bold: \\textbf{TRANS4}\index{ADVAN3}\index{TRANS}
  "ADVAN3", "TRANS5", "AOB, ALPHA, BETA",
  "ADVAN3", "TRANS6", "ALPHA, BETA, K21",
  "ADVAN4", "TRANS1", "K, K23, K32, KA",
  "ADVAN4", "TRANS3", "CL, V, Q, VSS, KA",
  "ADVAN4", "TRANS4", "CL, V2, Q, V3, KA", # bold: \\textbf{TRANS4}\index{ADVAN4}\index{TRANS}
...
readxl::read_excel("data-raw/fig-tab.xlsx", sheet="47", range="A2:D31") %>%
  rename(`ADVAN TRANS` = 1) %>%  \index{TRANS}
...
```r
$SUBROUTINES ADVAN4 TRANS4\index{\$SUBROUTINE}\index{ADVAN4}\index{TRANS}
$PK \index{\$PK}
  TVCL = THETA(1)
  TVV2 = THETA(2)
  TVV3 = THETA(3)
  ...
  ALAG1=  THETA(6) * EXP(ETA(6))\index{ALAG}
  S2   =  V2 / 1000
```
...
```r
IF (DOSE.EQ.50) F1 = 1\index{F1}
IF (DOSE.EQ.100) F1 = THETA(7)\index{F1}
IF (DOSE.EQ.200) F1 = THETA(8)\index{F1}
```
...
readxl::read_excel("data-raw/fig-tab.xlsx", sheet="50", range="B2:E36") %>% 
  kable(booktabs=TRUE, escape=FALSE, caption = "(ref:basic-additional)") %>% 
  column_spec(1, width = "2cm") %>%
  column_spec(2, width = "2.5cm") %>%
  column_spec(3, width = "1.5cm") %>%
  column_spec(4, width = "7.0cm") %>%
...
advan <- c("ADVAN5 (General Linear)",\index{ADVAN5}
"ADVAN6 (General Nonlinear)",\index{ADVAN6}
"ADVAN7 (General Linear with Real Eigenvalues)",\index{ADVAN7}
"ADVAN8 (General Nonlinear Kinetics with Stiff Equations)",\index{ADVAN8}
"ADVAN9 (General Nonlinear Kinetics with Equilibrium Compartments)",\index{ADVAN9}
"ADVAN13 (General Nonlinear Kinetics using LSODA)")\index{ADVAN1}\index{ADVAN13}
...
```{r advan-example, fig.cap="(ref:advan-lin-example)"}
knitr::include_graphics("./figures/05-fig-02.png")
```
...
```{r nonlin-advan-code-example, fig.cap="(ref:nonlin-advan-code-example)"}
knitr::include_graphics("./figures/05-fig-04.png")
```
...
$MODEL\index{\$MODEL}
  ...
  COMP(CENTRAL, DEFOBS)\index{COMP}\index{DEFOBS}\index{FO}
  COMP(PERIPH)\index{COMP}
$PK ; Define basic PK relationships\index{\$PK}
  CL = THETA(1)*EXP(ETA(1))
  V2 = THETA(2)*EXP(ETA(2))
  V3 = THETA(3)*EXP(ETA(3))
  Q  = THETA(4)*EXP(ETA(4))
...
$PK ; Define basic PK relationships\index{\$PK}
  ...
  K23= Q/V2
  K32= Q/V3
$DES ; Define differential equations\index{\$DES}
  DADT(1) = -A(1)*KA
...
$DATA ../sigmoidal_effect.csv IGNORE=@\index{\$DATA}\index{IGNORE}
$INPUT ID CP RESP=DV\index{\$INPUT}\index{ID}\index{RES}
$PRED\index{\$PRED}\index{PRED}
  E0=THETA(1) ; Baseline
...
![](./assets/media-10/image1.png) <!--no-num-->

![](./assets/media-10/image2.png) <!--no-num-->

![](./assets/media-10/image3.png) <!--no-num-->
...
![](./assets/media-10/image4.png) <!--no-num-->
...
![](./assets/media-10/image5.png) <!--no-num-->

![](./assets/media-10/image6.png) <!--no-num-->
...
![](./assets/media-10/image7.png) <!--no-num-->
...
![](./assets/media-10/image8.png) <!--no-num-->

![](./assets/media-10/image9.png) <!--no-num-->
...
$TABLE ID AGE WT HT                            ONEHEADER NOPRINT FILE=cotab3\index{\$TABLE}\index{FILE=}\index{ID}\index{NOPRINT}\index{ONEHEADER}
```
...
```{r workflow-covariate, fig.cap="(ref:workflow-covariate)"}
...
```{r covariate-summary, fig.cap="(ref:covariate-summary)"}
...
본 과정은 다음 [11.2.3](#colinearity)에서 소개할 공선성 분석을 위한 준비 단계로서, 각 대상자 별로 확보된 잠재적 공변량의 값들 간에 나타나는 비례관계 유무를 파악하는 것이다. 우선적으로는 시각적 방법이 활용되며, 통계적 방법을 통해 이를 보강할 수 있다. 이 과정의 핵심적 절차는 각 대상자에서 관찰된 잠재적 공변량 항목들의 값을 그림 \@ref(fig:cov-scatter)와 같이 scatterplot matrix로 나타내는 것이다. [11.2.1.1](#variable-eval)에서 언급한 바와 마찬가지로, xpose4 (ver. 4.5.X 기준)의 6: Covariate model \> 3. Scatterplot matrix of covariates 기능을 통해 간단히 수행할 수 있다. 그 수행 결과는 아래 그림 \@ref(fig:cov-scatter)에 나타난 바와 같다.
...
readxl::read_excel("data-raw/fig-tab.xlsx", sheet="112", range="B2:E6") %>% 
    ...
    kable(escape=FALSE, 
      ...
      booktabs=TRUE) %>% 
    add_header_above(c(" ", `$\\Delta$-2LL` = 3))
```
...
왼쪽의 경우와 오른쪽의 경우 ETA(i)의 의미는 어떻게 다르겠는가? 왼쪽의 경우, 분포용적에 대해 제공된 공정효과는 집단의 대표값인 THETA(i) 뿐이다. 따라서, 이 값과 각 개인의 분포용적 값 차이는 모두 ETA(i)로 설명된다. 이제 오른쪽을 보면, 고정효과로서 체중과의 선형적 관계가 제시되었다. 즉, 체중이 큰 사람은 분포용적이 크다는 일반적 경향은 고정효과로 설명이 되는 것이다. 결국 이 경우 ETA(i)는 체중의 차이가 무시된 집단의 대표값과 각 개인 값의 차이가 아닌, 같은 체중을 가진 사람들의 대표값과 각 개인 값의 차이가 된다. 따라서, 각 ETA(i)의 크기는 왼쪽에 비해 오른쪽에서 줄어들게 되고, 이는 궁극적으로 ω~i2~의 감소 효과를 일으키게 되는 것이다. 한 가지 추가할 사항은 시각적 스크리닝에서 본 바와 같은 공변량과 개인 별 파라미터 값 간의 scatterplot에 대한 것이다. 위에서 설명한 이유로 왼쪽의 ETA(i)는 체중과 일관된 경향을 가지는 값이 될 것이나, 오른쪽의 ETA(i)는 체중과 무관한 경향을 가지는 값이 되어야 한다.(체중의 영향은 고정효과로 설명이 되고, 남은 개인 간 차는 체중과는 무관한 차이일 것이므로) 따라서, 오른쪽 상황에서 체중과 ETA(i) 간의 scatterplot을 그리면 경향성이 없게 되는 것이 정상이며, 만일 경향성이 보인다면 공변량-파라미터 관계가 최적의 구조로 구현되지 않았다는 것을 의미하는 결과라 할 수 있다. 이 경우에는, [11.2.4.2](#cov-param)를 참고하여 새로운 공변량-파라미터 구조 모델을 시도해 보아야 한다.
...
[11.2.4.1](#forward-sel-def)에서 선형적 관계로 공변량을 반영하는 예시를 제시하였다. 많은 경우에 이러한 선형적 모델은 공변량 평가의 기본 구조가 된다. 그러나, 시각적 스크리닝 단계에서 선형적 관계가 아닌 것으로 보이거나, GAM 과정에서 2번 이상 선택되는 잠재적 공변량 등은 선형적 관계 이외에 보다 적합한 공변량-파라미터 관계를 가질 수 있다. 심지어 이러한 잠재적 공변량들은 단순 선형 모델을 이용한 공변량 반영 및 평가 시 [11.2.4.1](#forward-sel-def)에 제시한 3가지 기준을 만족하지 못할 수 있으므로, 해당 잠재적 공변량들에 대해서는 선형적인 관계로 공변량의 유의성을 평가만을 수행해서는 안 되며, 이후 또다른 구조를 이용한 평가가 필요하다. 아래는 연속형 변수로 측정된 공변량에 대해 적용할 수 있는 다양한 구조들의 예시이다.
...
위의 예에서 SEX는 성별을 뜻하며, 여성은 0, 남성은 1의 방식으로 coding할 수 있다. 만일, 특정 공변량이 3개 이상의 범주를 가지는 경우에는 위의 예와 같이 일관된 수식으로서 표현하기 어려운 상황이 많아, 제어구문 중 IF문을 사용하여, 각 범주 별로 파라미터 값을 각각 추정하게 할 수도 있다. 이 때 범주 하나 당 하나의 THETA가 대응되므로, [11.2.4.1](#forward-sel-def)의 1. 기준 평가 시 이를 고려해야 한다. 또한, 범주 별로 분산을 같다고 가정할 지, 다르다고 가정할 지 역시 모두 OFV 등의 기준을 이용하여 평가해 보아야 한다는 어려움도 있다. 범주형 변수가 가진 수준의 수가 충분히 많아지면, 이를 연속형 변수와 유사하게 처리할 수 있다는 이론도 있으나, 일반적인 모델링에서는 용량(증량 시험 등에서)을 제외하고 이렇게 다양한 수준의 범주형 자료가 수집되는 경우는 많지 않아 여기에서는 다루지 않는다.
...
```{r pred-sd, fig.cap="Relationship between standard deviation of residual variability and predicted concentrations"}
...
$ERROR\index{\$ERROR}
  IPRED = F\index{IPRED}\index{PRED}
  Y     = IPRED * (1+EPS(1)) + EPS(2)\index{IPRED}\index{PRED}
```
...
$ERROR\index{\$ERROR}
  IPRED = F\index{IPRED}\index{PRED}
  W     = SQRT(THETA(5)**2 + THETA(6) **2 * IPRED**2)\index{IPRED}\index{PRED}
  IRES  = DV - IPRED\index{IPRED}\index{IRES}\index{PRED}\index{RES}
  IWRES = IRES / W\index{IRES}\index{IWRES}\index{RES}\index{WRES}
  Y     = IPRED + W * EPS(1)\index{IPRED}\index{PRED}
```
...
Xpose4 package를 이용하여 여러 진단 플롯들을 그릴 수 있는데, 그 중 하나인 basic goodness-of-fit plots에서(그림 \@ref(fig:basic-gof-plot)) 아래쪽 두 개의 그림이 잔차(residual)에 대한 진단 플롯이다. 좌측은 |IWRES| vs Individual predictions (IPRED), 우측은 Conditional weighted residuals (CWRES) vs Time을 나타낸 그림이다. IWRES는 개인 가중잔차(Individual Weighted residuals)이고 |IWRES|는 개인 가중잔차의 절대값이다. 잔차모델이 적절하다면, CWRES 플롯의 모든 점들이 0을 기준으로 대칭으로 분포하게 되며, CWRES 값들의 흩어진 정도가 시간이나 개인 예측 값에 따라 경향성이 없고, 그 범위가 -3과+3 사이에 분포할 것으로 기대된다([10장](#use-of-xpose4) 참고).\index{개인 예측 / individual(specific) prediction(s)}\index{individual(specific) prediction(s) / 개인 예측}\index{잔차 / residual error}\index{residual error / 잔차}\index{conditional weighted residuals / 조건부 가중 잔차}\index{CWRES}\index{IPRED}\index{IWRES}\index{PRED}\index{RES}\index{WRES}

```{r basic-gof-plot, fig.cap = "Basic goodness-of-fit plots"}
...
```{r wrong-structure, fig.cap = "(ref:wrong-structure)"}
...
```{r lag-gof, fig.cap="Lag time 추가시의 GOF 변화"}
...
```{r u-curve, fig.cap = "Example of Wrong structure model (2-compartment model)"}
...
```{r addi-comb-gof, fig.cap = "Example of Wrong variance model (error model)"}
include_graphics("./figures/13-fig-10.png") # fig 11
```
...
read_csv('data-raw/media-13-tab-1.csv', col_types="cccc") %>% 
  ...
  add_header_above(c(" " = 1, `Plot of $\\epsilon_{i}$ versus` = 3), escape = FALSE)
```
...
$INPUT ID AMT=DROP TIME DV BWT\index{\$INPUT}\index{AMT}\index{DROP}\index{ID}\index{TIME}
$DATA ../THEO\index{\$DATA}
$PRED\index{\$PRED}\index{PRED}
  DOSE = 320
  KA   = THETA(1) * EXP(ETA(1))
  V    = THETA(2) * EXP(ETA(2))
  K    = THETA(3) * EXP(ETA(3))
  F    = DOSE/V*KA/(KA - K)*(EXP(-K*TIME) - EXP(-KA*TIME))\index{TIME}
  IPRE = F
  Y    = F + F*EPS(1) + EPS(2)

$THETA (0, 2) (0, 50) (0, 0.1)\index{\$THETA}
$OMEGA BLOCK(3)\index{\$OMEGA}\index{\$OMEGA BLOCK}
  0.2
  0.1  0.2
  0.1  0.1  0.2
$SIGMA 0.1  0.1\index{\$SIGMA}
$EST MAX=9999 METHOD=ZERO POSTHOC\index{\$ESTIMATION (\$EST)}\index{POSTHOC}
$COV PRINT=ERS\index{\$COVARIANCE (\$COV, \$COVAR)}\index{PRINT=E}
$TAB ID TIME IPRE CWRES\index{CWRES}\index{ID}\index{RES}\index{TIME}\index{WRES}
     FILE=sdtab NOPRINT ONEHEADER\index{FILE=}\index{NOPRINT}\index{ONEHEADER}
$TAB ID ETA(1) ETA(2) ETA(3)\index{ID}
     FILE=IndiEta.txt NOAPPEND ONEHEADER FIRSTONLY\index{FILE=}\index{FIRSTONLY}\index{NOAPPEND}\index{ONEHEADER}
```
...
\$COV output의 다섯 번째 부분은 상관행렬로부터 구한 고유치(eigenvalue)들을 작은 것부터 순서대로 나열한 것이다.  만약 상관행렬이 positive definite(양정치)라면 이것은 모두 양의 실수로 나와야 한다. 여기에 대한 추가 설명은 [14.5](#Theoph)를 참고한다.\index{상관행렬 / correlation matrix}\index{correlation matrix / 상관행렬}\index{\$COVARIANCE (\$COV, \$COVAR)}
...
function () 
{
  Smat = matrix(rep(0, e$nPara * e$nPara), nrow = e$nPara, 
                ncol = e$nPara)
  for (i in 1:e$nID) {\index{ID}
    e$DATAi = e$DataRef[[i]]\index{\$DATA}
    ...
    if (e$METHOD == "ZERO") {
      ...
      gr = grad(OiS1, e$FinalPara)\index{S1}
    }
    Smat = Smat + gr %*% t(gr)
  }
  return(Smat/4)
...
```r
DataAll = Theoph
colnames(DataAll) = c("ID", "BWT", "DOSE", "TIME", "DV")\index{ID}\index{TIME}
DataAll[,"ID"] = as.numeric(as.character(DataAll[,"ID"]))\index{ID}
...
FGD1 = deriv(~DOSE/(TH2*exp(ETA2))*TH1*exp(ETA1)/\index{D1}
             (TH1*exp(ETA1) - TH3*exp(ETA3))*
			 (exp(-TH3*exp(ETA3)*TIME)-exp(-TH1*exp(ETA1)*TIME)),\index{TIME}
             c("ETA1","ETA2","ETA3"),
             function.arg=c("TH1", "TH2", "TH3", "ETA1", "ETA2", "ETA3", 
			                "DOSE", "TIME"),\index{TIME}
             func=TRUE, hessian=FALSE)

H = deriv(~F + F*EPS1 + EPS2, c("EPS1", "EPS2"), \index{S1}
          function.arg=c("F", "EPS1", "EPS2"), \index{S1}
          func=TRUE)

PRED1 = function(THETA, ETA, DATAi)  # for FO and FOCE\index{first-order conditional estimation method(FOCE) / 1차 조건부 추정방법(FOCE)}\index{first-order method(FO) / 1차추정법}\index{D1}\index{FOCE}\index{FO}\index{PRED}
{
  FGDres = FGD1(THETA[1], THETA[2], THETA[3], ETA[1], ETA[2], ETA[3], DOSE=320, \index{D1}
                DATAi[,"TIME"])\index{TIME}
...
InitStep(DataAll, THETAinit=THETAinit, OMinit=OMinit, SGinit=SGinit, 
         LB=LB, UB=UB, Pred=PRED1, METHOD="ZERO")\index{D1}\index{PRED}
(EstRes1 = EstStep())
## $`Initial OFV`
## [1] 141.3076
## 
...
```{r purpose-eval}
readxl::read_excel("data-raw/fig-tab.xlsx", sheet="16-1", range="B2:D10") %>% \index{D1}
...
[18.5](#direct-pkpd)의 예시는 약물을 IV 로 투여 시 약동학이 1 구획 PK 모델을 따르고, 약력학은 Emax 모델을 따르는 경우이다. PK 와 PD가 시간지연 양상을 보이지 않으며, 직접효과를 나타내는 모델로 설명되었다.\index{직접 효과 모델 / direct-effect model}\index{direct-effect model / 직접 효과 모델}
...
Y1 = CP 
Y2 = EMAX*CP / (EC50+CP) 
IF (CMT.LE.1) Y = Y1 + ERR(1) \index{CMT}
IF (CMT.EQ.2) Y = Y2 + ERR(2)\index{CMT}
...
$INPUT ID TIME AMT DV MDV CMT \index{\$INPUT}\index{AMT}\index{CMT}\index{ID}\index{MDV}\index{TIME}
$DATA DATAFILENAME.csv IGNORE=@ \index{\$DATA}\index{IGNORE}
$SUBROUTIN ADVAN6 TOL=3 \index{ADVAN6}\index{TOL}
$MODEL \index{\$MODEL}
COMP (CENT, DEFDOSE, DEFOBS) \index{COMP}\index{DEFDOS}\index{DEFOBS}\index{FO}
COMP (PD)\index{COMP}
```
...
```r
$ERROR \index{\$ERROR}
IF (CMT.EQ.1) THEN \index{CMT}
  IPRED = CP \index{IPRED}\index{PRED}
  W = SQRT(THETA(10)**2 + THETA(11)**2 * IPRED**2) \index{IPRED}\index{PRED}
  IRES = DV - IPRED \index{IPRED}\index{IRES}\index{PRED}\index{RES}
  IWRES = IRES / W \index{IRES}\index{IWRES}\index{RES}\index{WRES}
  Y = IPRED + W * EPS(1) \index{IPRED}\index{PRED}
IF (CMT.EQ.2) THEN \index{CMT}
  IPRED = EMAX*CP/(EC50+CP) \index{IPRED}\index{PRED}
  W = THETA(12) 
  IRES = DV - IPRED \index{IPRED}\index{IRES}\index{PRED}\index{RES}
  IWRES = IRES / W \index{IRES}\index{IWRES}\index{RES}\index{WRES}
  Y = IPRED + W * EPS(2) \index{IPRED}\index{PRED}
ENDIF
```
...
```{r PPP-dataset, fig.cap = "(ref:PPP-dataset)"}
...
```{r sim-pppd-dataset, fig.cap = "(ref:sim-pppd-dataset)", out.width='50%'}
...
```r
$PROB HO_PO_2comp
$DATA SIM_S.csv IGNORE=@\index{\$DATA}\index{IGNORE}
$INPUT ID TIME AMT DMT DV MDV SEX AGE WT HT\index{\$INPUT}\index{AMT}\index{ID}\index{MDV}\index{TIME}

$SUBROUTINE ADVAN6 TOL=3\index{\$SUBROUTINE}\index{ADVAN6}\index{TOL}

$MODEL NCOMP=4\index{\$MODEL}\index{COMP}
  COMP(DEPOT, DEFDOSE)\index{COMP}\index{DEFDOS}
  COMP(CENT)\index{COMP}
  COMP(PERI)\index{COMP}
  COMP(EFFE, DEFOBS)\index{COMP}\index{DEFOBS}\index{FO}

$PK\index{\$PK}
;---- PK FIXED EFFECT ----\index{FIXED}
  TVCL = THETA(1)
  TVV2 = THETA(2)
  TVV3 = THETA(3)
  TVQ  = THETA(4)
...
```{r ppp-pkpd-dataset, fig.cap ="(ref:ppp-pkpd-dataset)"}
...
```{r ipp-dataset, fig.cap="(ref:ipp-dataset)"}
...
```r
$PROB HO_PO_2comp
$DATA IPP_S.csv IGNORE=@\index{\$DATA}\index{IGNORE}
$INPUT ID TIME AMT CMT DV MDV SEX AGE WT HT ICL IV2 IV3 IKA\index{\$INPUT}\index{AMT}\index{CMT}\index{ID}\index{MDV}\index{TIME}
...
```{r linear-pd, fig.cap="(ref:linear-pd)", out.width = '50%'}
...
[19.2](#pd-model)에서 PD 모델의 종류에 대하여 알아보았다. 몇 가지 예를 통하여 데이터의 종류와 상황에 따라 PD 모델을 어떻게 적용할지
살펴보자.
...
```r
$SUBROUTINES ADVAN3\index{\$SUBROUTINE}\index{ADVAN3}
$PK\index{\$PK}
  K   = THETA(1)*EXP(ETA(1))        ; TRIVIAL LOSS TO EFFECT CMPT\index{SS}
  K12 = .001*K                      ; KEO
  K21 = THETA(2)*EXP(ETA(2))        ; V1 FOR DRUG\index{FO}
  S1  = THETA(3)*EXP(ETA(3))\index{S1}
  EMAX= THETA(4)*EXP(ETA(4))
  C50 = THETA(5)*EXP(ETA(5))
  S2  = S1*K12/K21                  ; SO THAT CESS = CPSS\index{S1}\index{SS}
$ERROR\index{\$ERROR}
  IF (CMT.EQ.2) THEN\index{CMT}
  Y   = EMAX*F/(C50+F)*EXP(ERR(2))  ; DV = EFFECT IF CMT=2\index{CMT}
  ELSE
  Y   = F*EXP(ERR(1))               ; DV = CP IF CMT=1 (OR 0)\index{CMT}
  ENDIF
```
...
```r
$SUBROUTINES ADVAN6 TOL=5\index{\$SUBROUTINE}\index{ADVAN6}\index{TOL}
$MODEL\index{\$MODEL}
  COMP=(CENTRAL,DEFDOSE,DEFOBS)\index{COMP}\index{DEFDOS}\index{DEFOBS}\index{FO}
  COMP=EFFECT\index{COMP}
$PK\index{\$PK}
...
$DES\index{\$DES}
  DADT(1)=-K10*A(1)                 ; NOTE NO LOSS TO A(2)\index{SS}
