...
```r
$PROB HO_IV_1comp
$DATA ../HOdata_IV.csv IGNORE=@\index{$DATA}\index{IGNORE}
$INPUT ID TIME AMT DV MDV SEX AGE WT HT\index{$INPUT}\index{AMT}\index{ID}\index{MDV}\index{TIME}
```
...
## PREDPP에 쓰는 데이터셋\index{PRED}\index{PREDPP}
...
```{r reserveditems}
readxl::read_excel("data-raw/fig-tab.xlsx", sheet='16', range="B2:D12") %>% \index{D1}
...
```r
DATE  month day year\index{DATE}
DAT1  day month year\index{DAT1}
DAT2  year month day\index{DAT2}
DAT3  year day month\index{DAT3}
```
